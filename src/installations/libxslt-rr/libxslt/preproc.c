...
xsltNewStylePreComp(xsltStylesheetPtr style, xsltStyleType type) {
    ...
    */
    switch (type) {
        ...
        case XSLT_FUNC_ELSE:
            size = sizeof(xsltStyleItemElse); break;
        case XSLT_FUNC_ELSEIF:
            size = sizeof(xsltStyleItemElseIf); break;
    ...
    */
    ...
    switch (cur->type) {
        ...
        case XSLT_FUNC_ELSE:
            cur->func = (xsltTransformFunction) xsltElse;break;
        case XSLT_FUNC_ELSEIF:
            cur->func = (xsltTransformFunction) xsltElseIf;break;
        case XSLT_FUNC_FOREACH:
    ...
    */
	...
	case XSLT_FUNC_COPY:
        ...
        case XSLT_FUNC_ELSE:
            break;
        case XSLT_FUNC_ELSEIF: {
                xsltStyleItemElseIfPtr item = (xsltStyleItemElseIfPtr) comp;
                if (item->comp != NULL)
                    xmlXPathFreeCompExpr(item->comp);
            }
            break;
	...
	*/
	...
	if (inst->ns != NULL) {
		...
		*/
		...
		XSLT_TRACE_GENERIC(XSLT_TRACE_EXTRA, "Found xslt11:document construct");
	    } else {
		if (xmlStrEqual(inst->ns->href,
		    (const xmlChar *)"http://exslt.org/common")) {
		    /* EXSLT. */

		    XSLT_TRACE_GENERIC(XSLT_TRACE_EXTRA, "Found exslt:document extension");
		} else if (xmlStrEqual(inst->ns->href, XSLT_XT_NAMESPACE)) {
		    /* James Clark's XT. */

		    XSLT_TRACE_GENERIC(XSLT_TRACE_EXTRA, "Found xt:document extension");
		}
	    }
	}
	/*
    ...
    */
	...
	if (xmlValidateQName(comp->name, 0)) {
	    ...
	    const xmlChar *prefix = NULL, *name;
            UNUSED(name); /* Annesley: ok if variable not used (XSLT_REFACTORED) */
	...
	if (xmlValidateQName(comp->name, 0)) {
		...
		if (!xmlStrncasecmp(prefix, (xmlChar *) "xmlns", 5)) {
		    ...
		    */
		    ...
		    XSLT_TRACE_GENERIC(XSLT_TRACE_PARSING, "xsltAttribute: xmlns prefix forbidden");
		    return;
		}
...
xsltWithParamComp(xsltStylesheetPtr style, xmlNodePtr inst) {
    ...
    * Attribute "name".
    */
    xsltGetQNameProperty(style, inst, BAD_CAST "name", 1, &(comp->has_name), &(comp->ns), &(comp->name));
    if (comp->ns)
	comp->has_ns = 1;
...
xsltApplyTemplatesComp(xsltStylesheetPtr style, xmlNodePtr inst) {
     ...
     * Attribute "mode".
     */
    xsltGetQNameProperty(style, inst, BAD_CAST "mode", 0, NULL, &(comp->modeURI), &(comp->mode));
    
    /*
    * Attribute "select".
    */
    comp->select = xsltGetCNsProp(style, inst, BAD_CAST "select", XSLT_NAMESPACE);
    if (comp->select != NULL) {
	comp->comp = xsltXPathCompile(style, comp->select);
	if (comp->comp == NULL) {
	    xsltTransformError(NULL, style, inst,
    ...
    /*
    * Annesley: Attribute "with-all-params".
    * xsltGetCNsProp() return is anchored in the DICT
    */
    prop = xsltGetCNsProp(style, inst, BAD_CAST "with-all-params", XSLT_NAMESPACE);
    if ((prop != NULL) && (xmlStrEqual(prop, BAD_CAST "all") == 1)) comp->withAllParams = 1;
}
 ...
 * xsltElseComp:
 * @style: an XSLT compiled stylesheet
 * @inst:  the xslt else node
 *
 * Process the xslt else node on the source node
 */
static void
xsltElseComp(xsltStylesheetPtr style, xmlNodePtr inst) {
#ifdef XSLT_REFACTORED
    xsltStyleItemElsePtr comp;
#else
    xsltStylePreCompPtr comp;
#endif
...
#ifdef XSLT_REFACTORED
    comp = (xsltStyleItemElsePtr)
        xsltNewStylePreComp(style, XSLT_FUNC_ELSE);
#else
    comp = xsltNewStylePreComp(style, XSLT_FUNC_ELSE);
#endif
 ...
 * xsltElseIfComp:
 * @style: an XSLT compiled stylesheet
 * @inst:  the xslt else-if node
 *
 * Process the xslt else-if node on the source node
 */
static void
xsltElseIfComp(xsltStylesheetPtr style, xmlNodePtr inst) {
#ifdef XSLT_REFACTORED
    xsltStyleItemElseIfPtr comp;
#else
    xsltStylePreCompPtr comp;
#endif
...
#ifdef XSLT_REFACTORED
    comp = (xsltStyleItemElseIfPtr)
        xsltNewStylePreComp(style, XSLT_FUNC_ELSEIF);
#else
    comp = xsltNewStylePreComp(style, XSLT_FUNC_ELSEIF);
#endif
    ...
    if (comp->test == NULL) {
        xsltTransformError(NULL, style, inst,
             "xsl:else-if : test is not defined\n");
        if (style != NULL) style->errors++;
        return;
    }
    comp->comp = xsltXPathCompile(style, comp->test);
    if (comp->comp == NULL) {
        xsltTransformError(NULL, style, inst,
             "xsl:else-if : could not compile test expression '%s'\n",
                         comp->test);
        if (style != NULL) style->errors++;
    }
}
...
xsltVariableComp(xsltStylesheetPtr style, xmlNodePtr inst) {
    ...
    comp = (xsltStyleItemVariablePtr) xsltNewStylePreComp(style, XSLT_FUNC_VARIABLE);
    if (comp == NULL)
	return;
...
xsltTextComp(xsltStylesheetPtr style, xmlNodePtr inst) {
    ...
    prop = xsltGetCNsProp(style, inst, (const xmlChar *)"disable-output-escaping", XSLT_NAMESPACE);
    if (prop != NULL) {
        if (xmlStrEqual(prop, (const xmlChar *)"yes")) {
            comp->noescape = 1;
        } else if (!xmlStrEqual(prop, (const xmlChar *)"no")) {
            xsltTransformError(NULL, style, inst, "xsl:text: disable-output-escaping allows only yes or no\n");
            if (style != NULL) style->warnings++;
        }
    }
}
...
xsltParamComp(xsltStylesheetPtr style, xmlNodePtr inst) {
    xsltStyleItemParamPtr comp;
    const xmlChar *selectWhenBlank;

    if ((style == NULL) || (inst == NULL))
	return;

    comp = (xsltStyleItemParamPtr) xsltNewStylePreComp(style, XSLT_FUNC_PARAM);
    if (comp == NULL)
	return;
    inst->psvi = comp;
    comp->inst = inst;
    ...
    */
    ...
    /*
     * Attribute "selectWhenBlank".
     * Annesley: XSLT defaults params to blank strings if they are not sent
     * we allow the reverse here
     * xsltGetCNsProp() return is anchored in the DICT
     */
    selectWhenBlank = xsltGetCNsProp(style, inst, (const xmlChar *)"select-when-blank", XSLT_NAMESPACE);
    if (selectWhenBlank != NULL) {
      comp->selectWhenBlank = xmlStrEqual(selectWhenBlank, BAD_CAST "yes");
    }
}
...
xsltStylePreCompute(xsltStylesheetPtr style, xmlNodePtr node) {
    ...
    */
	...
	switch (XSLT_CCTXT(style)->inode->type) {
		...
		break;
            case XSLT_FUNC_ELSE:
                xsltElseComp(style, node);
                break;
            case XSLT_FUNC_ELSEIF:
                xsltElseIfComp(style, node);
                break;
	    case XSLT_FUNC_CHOOSE:
		...
		break;
            case XSLT_FUNC_TEXT:
                /* Annesley: re-inserted because disable-output-escaping needs to be set */
                xsltTextComp(style, node);
                break;
	    case XSLT_FUNC_PARAM:
	...
	*/
	if (IS_XSLT_NAME(node, "apply-templates")) {
        ...
        /* Annesley: re-inserted because disable-output-escaping needs to be set */
        } else if (IS_XSLT_NAME(node, "text")) {
            xsltTextComp(style, node);
