...
xsltPrintErrorContext(xsltTransformContextPtr ctxt,
	              xsltStylesheetPtr style, xmlNodePtr node, const char *msg) {
    int line = 0;
    ...
    /* Annesley: extra msg processing to ensure one sensible error in one go */
    if (msg == NULL) msg = "no details";
...
xsltTransformError(xsltTransformContextPtr ctxt,
		   ...
		   xmlNodePtr node,
		   const char *msg, ...) {
    /* Annesley: not using these "error" variables anymore:
     * xmlGenericErrorFunc error = xsltGenericError;
     * void *errctx = xsltGenericErrorContext;
     */
    char * str;
	...
	*/
    ...
    /* error(errctx, "%s", str); */
    xsltPrintErrorContext(ctxt, style, node, str); /* calls error() also, so str added */
    if (str != NULL)
	xmlFree(str);
}
...
xsltDocumentSortFunction(xmlNodeSetPtr list) {
	...
	for (j = i + 1; j < len; j++) {
		...
		list->nodeTab[j] = node;
                ...
                if (list->parent_routeTab != NULL) {
                  parent_route = list->parent_routeTab[i];
                  list->parent_routeTab[i] = list->parent_routeTab[j];
                  list->parent_routeTab[j] = parent_route;
                }
	    }
	}
    }
...
xsltComputeSortResult(xsltTransformContextPtr ctxt, xmlNodePtr sort) {
	...
	if (res != NULL) {
		...
		if (res->type == XPATH_NUMBER) {
		    ...
		    XSLT_TRACE_GENERIC(XSLT_TRACE_PROCESS, "xsltComputeSortResult: select didn't evaluate to a number");
		    results[i] = NULL;
		}
	    } else {
		if (res->type == XPATH_STRING) {
		    ...
		    XSLT_TRACE_GENERIC(XSLT_TRACE_PROCESS, "xsltComputeSortResult: select didn't evaluate to a string");
		    results[i] = NULL;
		}
	    }
	} else {
 ...
 */
void
xsltDefaultSortFunction(xsltTransformContextPtr ctxt, xmlNodePtr *sorts, int nbsorts) {
#ifdef XSLT_REFACTORED
    xsltStyleItemSortPtr comp;
#else
    xsltStylePreCompPtr comp;
	...
	if ((comp->stype == NULL) && (comp->has_stype != 0)) {
	    comp->stype =
		xsltEvalAttrValueTemplate(ctxt, sorts[j], ctxt->xpathCtxt->parent_route,
	...
	if ((comp->order == NULL) && (comp->has_order != 0)) {
	    comp->order = xsltEvalAttrValueTemplate(ctxt, sorts[j], ctxt->xpathCtxt->parent_route,
						    (const xmlChar *) "order",
						    XSLT_NAMESPACE);
	    if (comp->order != NULL) {
		temporder[j] = 1;
	...
	for (i = incr; i < len; i++) {
		...
		if (tst > 0) {
		    ...
		    list->nodeTab[j + incr] = node;
                    ...
                    if (list->parent_routeTab != NULL) {
                      parent_route = list->parent_routeTab[j];
                      list->parent_routeTab[j] = list->parent_routeTab[j + incr];
                      list->parent_routeTab[j + incr] = parent_route;
                    }
...
xsltSaveResultTo(xmlOutputBufferPtr buf, xmlDocPtr result,
    ...
    } else if ((method != NULL) && (xmlStrEqual(method, (const xmlChar *) "xhtml"))) {
	if (encoding != NULL) {
	    htmlSetMetaEncoding(result, (const xmlChar *) encoding);
	} else {
	    htmlSetMetaEncoding(result, (const xmlChar *) "UTF-8");
	...
	xmlOutputBufferFlush(buf);
    } else if ((method != NULL) && (xmlStrEqual(method, (const xmlChar *) "text"))) {
	xmlNodePtr cur;
	...
	if (result->children != NULL) {
	    ...
	    while (child != NULL) {
		xmlNodeDumpOutput(buf, result, child, 0, (indent == 1), (const char *) encoding, 0);
...
xsltXPathCompile(xsltStylesheetPtr style, const xmlChar *str) {
    ...
    if (style != NULL) {
        ...
        /* Annesley: XSLT_REFACTORED_XPATHCOMP is ON in xsltInternals.h */
	if (XSLT_CCTXT(style)) {
	    /*
	...
	xpathCtxt->dict = style->dict;
        ...
        /* Annesley: grammar processing */
        if (style->gp) xpathCtxt->gp = style->gp;
    } else {
