...
xsltAddKey(xsltStylesheetPtr style, const xmlChar *name,
    ...
    XSLT_TRACE_GENERIC3(XSLT_TRACE_KEYS, "Add key %s, match %s, use %s", name, match, use);
    ...
    XSLT_TRACE_GENERIC1(XSLT_TRACE_KEYS, "   resulting pattern %s", pattern);
    /*
...
xsltGetKey(xsltTransformContextPtr ctxt, const xmlChar *name,
    ...
    XSLT_TRACE_GENERIC2(XSLT_TRACE_KEYS, "Get key %s, value %s", name, value);
...
fprintf(stderr, "xsltInitCtxtKey %s : %d\n", keyDef->name, ctxt->keyInitLevel);
	...
	*/
	...
	while (1) {
	    ...
	    if (keylist == NULL) {
		keylist = xmlXPathNodeSetCreate(cur, NULL);
		if (keylist == NULL)
		    goto error;
		...
		*/
		xmlXPathNodeSetAdd(keylist, cur, NULL);
	    }
	    switch (cur->type) {
