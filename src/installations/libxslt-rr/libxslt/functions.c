...
xsltXPathFunctionLookup (xmlXPathContextPtr ctxt,
    ...
    XSLT_TRACE_DEFAULT(XSLT_TRACE_FUNCTION, xsltGenericDebug(xsltGenericDebugContext, "Lookup function {%s}%s\n", ns_uri, name));
    ...
    */
    ...
    if (ret != NULL) XSLT_TRACE_DEFAULT(XSLT_TRACE_FUNCTION, xsltGenericDebug(xsltGenericDebugContext, "found function %s\n", name));
...
xsltDocumentFunctionLoadDocument(xmlXPathParserContextPtr ctxt, xmlChar* URI)
{
    ...
    if (fragment == NULL) {
	valuePush(ctxt, xmlXPathNewNodeSet((xmlNodePtr) doc, NULL));
	return;
    }
    ...
    if (ctxt->value->type == XPATH_NODESET) {
        ...
        if (obj->nodesetval) {
            for (i = 0; i < obj->nodesetval->nodeNr; i++) {
                parent_route = (obj->nodesetval->parent_routeTab == NULL ? NULL : obj->nodesetval->parent_routeTab[i]);
                valuePush(ctxt, xmlXPathNewNodeSet(obj->nodesetval->nodeTab[i], parent_route));
                ...
                ret->nodesetval = xmlXPathNodeSetMerge(ret->nodesetval, newobj->nodesetval);
    ...
    */
	...
	if (obj2->nodesetval != NULL) {
	    for (i = 0; i < obj2->nodesetval->nodeNr; i++) {
                parent_route = (obj2->nodesetval->parent_routeTab == NULL ? NULL : obj2->nodesetval->parent_routeTab[i]);
		valuePush(ctxt, xmlXPathObjectCopy(obj1));
		valuePush(ctxt, xmlXPathNewNodeSet(obj2->nodesetval->nodeTab[i], parent_route));
		xmlXPathStringFunction(ctxt, 1);
...
xsltCurrentFunction(xmlXPathParserContextPtr ctxt, int nargs){
	...
	valuePush(ctxt, xmlXPathNewNodeSet(tctxt->node, NULL)); /* current */
    }
}
