...
xsltExtModuleRegisterDynamic(const xmlChar * URI)
{
	...
	if (NULL == ext_directory)
      ...
      XSLT_TRACE_GENERIC1(XSLT_TRACE_EXTENSIONS, "LIBXSLT_PLUGINS_PATH is %s", ext_directory);
    }
    ...
    XSLT_TRACE_GENERIC2(XSLT_TRACE_EXTENSIONS, "Attempting to load plugin: %s for URI: %s",
                     module_filename, URI);

    if (1 != xmlCheckFilename(module_filename)) {


	XSLT_TRACE_GENERIC1(XSLT_TRACE_EXTENSIONS, "xmlCheckFilename failed for plugin: %s", module_filename);
	...
	XSLT_TRACE_GENERIC1(XSLT_TRACE_EXTENSIONS, "xmlModuleOpen failed for plugin: %s", module_filename);
	...
	XSLT_TRACE_GENERIC2(XSLT_TRACE_EXTENSIONS, "xmlModuleSymbol failed for plugin: %s, regfunc: %s",
                      ...
                      const xmlChar * prefix, const xmlChar * URI)
{
    ...
    XSLT_TRACE_GENERIC1(XSLT_TRACE_EXTENSIONS, "Registering extension namespace '%s'.", URI);
				     ...
				     const xmlChar * URI)
{
	...
	XSLT_TRACE_GENERIC1(XSLT_TRACE_EXTENSIONS, "Not registered extension module: %s", URI);
	return(NULL);
    }
    ...
    */
	...
	XSLT_TRACE_GENERIC1(XSLT_TRACE_EXTENSIONS, "Initializing module with *no* callback: %s", URI);
    } else {

	XSLT_TRACE_GENERIC1(XSLT_TRACE_EXTENSIONS, "Initializing module with callback: %s", URI);
	/*
...
xsltGetExtData(xsltTransformContextPtr ctxt, const xmlChar * URI)
{
    ...
    if (data == NULL) {
        ...
        if (module == NULL) {
            ...
            XSLT_TRACE_GENERIC1(XSLT_TRACE_EXTENSIONS, "Initializing module: %s", URI);
                ...
                const xmlChar * URI)
{
    ...
    if ((styleData == NULL) || (ctxt == NULL) || (URI == NULL) ||
        ...
        XSLT_TRACE_GENERIC(XSLT_TRACE_EXTENSIONS, "xsltInitCtxtExt: NULL param or error");
        return;
    }
    module = styleData->extModule;
    if ((module == NULL) || (module->initFunc == NULL)) {

        XSLT_TRACE_GENERIC(XSLT_TRACE_EXTENSIONS, "xsltInitCtxtExt: no module or no initFunc");
        return;
    }
    ...
    if (ctxtData != NULL) {
        ...
        XSLT_TRACE_GENERIC(XSLT_TRACE_EXTENSIONS, "xsltInitCtxtExt: already initialized");
        return;
    }
    ...
    if (extData == NULL) {
        ...
        XSLT_TRACE_GENERIC(XSLT_TRACE_EXTENSIONS, "xsltInitCtxtExt: no extData");
    }
    ...
    XSLT_TRACE_GENERIC1(XSLT_TRACE_EXTENSIONS, "Registered module %s",
                     URI);
    ctxt->ret++;
}
...
xsltInitCtxtExts(xsltTransformContextPtr ctxt)
{
    ...
    XSLT_TRACE_GENERIC1(XSLT_TRACE_EXTENSIONS, "Registered %d modules",
                     ctx.ret);
    return (ctx.ret);
}
                    ...
                    const xmlChar * URI)
{
    ...
    XSLT_TRACE_GENERIC1(XSLT_TRACE_EXTENSIONS, "Shutting down module : %s", URI);
                    ...
                    int nargs ATTRIBUTE_UNUSED)
{
    ...
    XSLT_TRACE_GENERIC1(XSLT_TRACE_FUNCTION, "libxslt:test() called with %d args", nargs);
}
 ...
 * @author: Annesley
 */
static void xmlAssembleExtensions(void * payload, void * data, const xmlChar * name, const xmlChar * name2, const xmlChar * name3) {
  xmlHashTablePtr h = (xmlHashTablePtr) data;
  xmlHashAddEntry(h, (void*) name2, (void*) name2); //HREF. returns -1 if fails
}

const xmlHashTablePtr xsltExtensionHREFs() {
    xmlHashTablePtr h;

    /* generate a unique hash of HREFs */
    if (xsltElementsHash == NULL) {
      h = xmlHashCreate(0);
    } else {
      h = xmlHashCreate(xmlHashSize(xsltElementsHash));
      xmlHashScanFull(xsltElementsHash, xmlAssembleExtensions, h);
    }
