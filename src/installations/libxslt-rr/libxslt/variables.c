 ...
 */
int
xsltRegisterLocalRVT(xsltTransformContextPtr ctxt, xmlDocPtr RVT) {
 ...
 * xmlCopyGlobalVar:
 * @author: Annesley
 * @payload: the hash table item
 * @name:  name of the item (not relevant here)
 *
 * (xmlHashCopier) Copy StackElem
 *
 * Returns the new table or NULL in case of error.
 */
void *xmlCopyGlobalVar(void *payload, xmlChar *name) {
    return xsltCopyStackElem(payload);
}
...
/**
 * xsltCopyStackElemList:
 * @author: Annesley
 * @ctxt:  xn XSLT transformation context
 * @elems:  a stack element list
 *
 * Copy Push an element list onto the stack.
 *
 * Returns 0 in case of success, -1 in case of failure.
 */
int xsltCopyStackElemList(xsltTransformContextPtr ctxt, xsltStackElemPtr elem)
{
    if ((ctxt == NULL) || (elem == NULL)) return(-1);

    do {
      if (xsltAddStackElemList(ctxt, xsltCopyStackElem(elem))) return (-1);
      elem = elem->next;
    } while (elem != NULL);
             ...
             xsltStylePreCompPtr castedComp)
{
    ...
    */
    ...
    XSLT_TRACE(ctxt,XSLT_TRACE_VARIABLES,xsltGenericDebug(xsltGenericDebugContext, "Evaluating variable '%s'\n", variable->name));
    ...
    */
    ...
    /* Annesley: we should never try to evaluate stuff without namespaces available */
    if (xpctxt->nsNr == 0) {
      xsltTransformError(ctxt, NULL, (comp != NULL) ? comp->inst : NULL,
        "Attempt to evaluate a variable without namespaces available '%s'.\n",
        variable->name);
    }
...
xsltEvalGlobalVariable(xsltStackElemPtr elem, xsltTransformContextPtr ctxt)
{
    ...
    oldInst = ctxt->inst;
    comp = (xsltStyleBasicItemVariablePtr) elem->comp; /* Annesley; ignoring non-XSLT_REFACTORED */
    ...
    */
    ...
    xpctxt->parent_route = ctxt->initialContextParentRoute;
    ...
    /* Annesley: GDB:
     *  ((xsltTransformContextPtr)xpctxt->externalContext)->inst->properties->next
     *  ((xsltTransformContextPtr)xpctxt->externalContext)->inst->properties->next->next->children->content
     */
    result = xmlXPathCompiledEval(xpExpr, xpctxt);
...
xsltRegisterGlobalVariable(xsltStylesheetPtr style, const xmlChar *name,
    ...
    if (comp->type == XSLT_FUNC_PARAM) {
        XSLT_TRACE_GENERIC1(XSLT_TRACE_VARIABLE, "Defining global param %s", name);
    } else {
        XSLT_TRACE_GENERIC1(XSLT_TRACE_VARIABLE, "Defining global variable %s", name);
    }
...
static
xsltStackElemPtr
xsltProcessUserParamInternal(xsltTransformContextPtr ctxt, const xmlChar * name, const xmlChar * value, int eval) {
    xsltStylesheetPtr style;
    ...
    /* Annesley: allow_global_var_override */
    int allow_global_var_override = 1;

    if (ctxt  == NULL) {
        xsltTransformError(ctxt, NULL, NULL, "Evaluating user parameters failed because context is NULL\n");
        return(NULL);
    }
    ...
    if (prefix != NULL) {
        xmlNsPtr ns;
        ns = xmlSearchNs(style->doc, xmlDocGetRootElement(style->doc), prefix);
        if (ns == NULL) {
    ...
    if (name == NULL) {
        xsltTransformError(ctxt, style, NULL, "Evaluating user parameters failed because name evaluated to NULL\n");
        return (NULL);
    }

    res_ptr = xmlHashLookup2(ctxt->globalVars, name, href);
    if (res_ptr != 0) {
       /* overriding of globalVars */
       if (allow_global_var_override == 0) xsltTransformError(ctxt, style, NULL, "Global parameter %s already defined\n", name);
    }
    if (ctxt->globalVars == NULL)
        ctxt->globalVars = xmlHashCreate(20);

    if (allow_global_var_override == 0)  {
      while (style != NULL) {
    ...
    if (elem != NULL) {
        ...
        if (href != NULL) elem->nameURI = xmlDictLookup(ctxt->dict, href, -1);
        elem->tree = NULL;
        elem->computed = 1;
        if (eval == 0)  elem->value = xmlXPathNewString(value);
        else            elem->value = result;
    }

    /*
     * Global parameters are stored in the XPath context variables pool.
     */
    if (allow_global_var_override == 1)
      res = xmlHashUpdateEntry2(ctxt->globalVars, name, href, elem, NULL);
    else
      res = xmlHashAddEntry2(ctxt->globalVars, name, href, elem);
    if ((res != 0) && (allow_global_var_override == 0)) {
      xsltFreeStackElem(elem);
...
xsltStackElemPtr
xsltEvalUserParams(xsltTransformContextPtr ctxt, const char **params, xsltStackElemPtr withParams) {
    int indx = 0;
    const xmlChar *name;
    const xmlChar *value;
    xsltStackElemPtr elem;
    ...
    while (params[indx] != NULL) {
        ...
        elem  = xsltEvalOneUserParam(ctxt, name, value);
        if (elem == NULL) {
            xsltTransformError(ctxt, NULL, NULL, "Evaluating user parameters failed\n");
            return(NULL);
        } else {
          /* add elem to return list */
          if (withParams != NULL) elem->next = withParams;
          withParams = elem;
        }
    }
...
/**
 * xsltEvalUserNodeSetParams:
 * Author: Annesley
 *
 * @ctxt:     the XSLT transformation context
 * @nodesets: existing nodesets pointers to push on to the globalVars HashTable
 *
 * Returns the end of the withParams list
 */
xsltStackElemPtr
xsltEvalUserNodeSetParams(xsltTransformContextPtr ctxt, const char **nodesets, xsltStackElemPtr withParams) {
    xsltStackElemPtr elem;
    int indx = 0;
    const xmlChar *href = NULL; /* namespace */
    const xmlChar *name;
    xmlNodeSetPtr nodeset;

    if (nodesets == NULL) return(0);
    
    while (nodesets[indx] != NULL) {
        name    = (const xmlChar *)     nodesets[indx++];
        nodeset = (const xmlNodeSetPtr) nodesets[indx++];
        elem    = xsltNewStackElem(ctxt);
        if (elem == NULL) {
            xsltTransformError(ctxt, NULL, NULL, "Evaluating user nodeset parameters failed\n");
            break;
        } else {
          elem->name     = 0;
          elem->value    = xmlXPathWrapNodeSet(nodeset);
          elem->computed = 1;
          /* add elem to return list */
          if (withParams != NULL) elem->next = withParams;
          withParams = elem;
          ...
          xmlHashUpdateEntry2(ctxt->globalVars, name, href, elem, NULL);
        }
    }
...
xsltStackElemPtr
xsltQuoteUserParams(xsltTransformContextPtr ctxt, const char **params, xsltStackElemPtr withParams) {
    xsltStackElemPtr elem;
    int indx = 0;
    const xmlChar *name;
    const xmlChar *value;
    ...
    while (params[indx] != NULL) {
        name  = (const xmlChar *) params[indx++];
        value = (const xmlChar *) params[indx++];
        elem  = xsltQuoteOneUserParam(ctxt, name, value);
        if (elem == NULL) {
            xsltTransformError(ctxt, NULL, NULL, "Quoting user parameters failed\n");
            break;
        } else {
          /* add elem to return list */
          if (withParams != NULL) elem->next = withParams;
...
xsltStackElemPtr
xsltEvalOneUserParam(xsltTransformContextPtr ctxt, const xmlChar * name, const xmlChar * value) {
    /* 1 = xpath eval ? */
    return xsltProcessUserParamInternal(ctxt, name, value, 1);
}
...
xsltStackElemPtr
xsltQuoteOneUserParam(xsltTransformContextPtr ctxt, const xmlChar * name, const xmlChar * value) {
    /* 0 = xpath eval ? */
    return xsltProcessUserParamInternal(ctxt, name, value, 0);
}
...
{
#ifdef XSLT_REFACTORED
    xsltStyleBasicItemVariablePtr comp = (xsltStyleBasicItemVariablePtr) castedComp;
#else
    xsltStylePreCompPtr comp = castedComp;
#endif
    xsltStackElemPtr elem;
    ...
    elem->tree = tree;
    elem->value = xsltEvalVariable(ctxt, elem, (xsltStylePreCompPtr) comp);
    ...
    */
    if (elem->computed == 0) {
      if (elem->name == xsltComputingGlobalVarMarker) {
          xsltTransformError(ctxt, NULL, elem->comp->inst, "Recursive definition of %s\n", name);
          return(NULL);
      }
 ...
 * xsltCopyVariable:
 * @ctxt:  the XSLT transformation context
 * @variable:  the context variable to pass through
 *   xsl:param children are not used because the template may also be @with-all-params
 *
 * Returns the new xsltStackElemPtr or NULL
 */
xsltStackElemPtr
xsltCopyVariable(xsltTransformContextPtr ctxt, xsltStackElemPtr variable)
{
    xsltStackElemPtr copy;
    
    copy = xsltNewStackElem(ctxt);
    if (copy == NULL) return(NULL);
    
    copy->comp    = variable->comp;
    copy->name    = variable->name;    /* no-dup because shared DICT */
    copy->select  = NULL;
    copy->nameURI = variable->nameURI; /* no-dup because shared DICT */
    copy->tree    = variable->tree;
    if (variable->value != NULL) {
      copy->value = xmlXPathObjectCopy(variable->value);
      copy->computed = 1;
    }
...
xsltParseStylesheetCallerParam(xsltTransformContextPtr ctxt, xmlNodePtr inst)
{
    ...
    XSLT_TRACE(ctxt,XSLT_TRACE_VARIABLES,xsltGenericDebug(xsltGenericDebugContext, "Handling xsl:with-param %s\n", comp->name));

    if (comp->select == NULL) {
      tree = inst->children;
    } else {
      XSLT_TRACE(ctxt,XSLT_TRACE_VARIABLES,xsltGenericDebug(xsltGenericDebugContext, "        select %s\n", comp->select));
      tree = inst;
    }
...
xsltParseGlobalVariable(xsltStylesheetPtr style, xmlNodePtr cur)
{
    ...
    */
    ...
    XSLT_TRACE_GENERIC1(XSLT_TRACE_VARIABLE, "Registering global variable %s", comp->name);

    xsltRegisterGlobalVariable(style, comp->name, comp->ns, comp->select, cur->children, (xsltStylePreCompPtr) comp, NULL);
}
...
xsltParseGlobalParam(xsltStylesheetPtr style, xmlNodePtr cur) {
    ...
    */
    ...
    XSLT_TRACE_GENERIC1(XSLT_TRACE_VARIABLE, "Registering global param %s", comp->name);
...
xsltParseStylesheetParam(xsltTransformContextPtr ctxt, xmlNodePtr cur)
{
    ...
    XSLT_TRACE(ctxt,XSLT_TRACE_VARIABLES,xsltGenericDebug(xsltGenericDebugContext, "Registering param %s\n", comp->name));
 ...
 */
xmlXPathObjectPtr
xsltXPathVariableLookup(void *ctxt, const xmlChar *name, const xmlChar *ns_uri) {
    ...
    */
    {
        ...
        if (ns_uri != NULL) ns_uri = xmlDictLookup(tctxt->dict, ns_uri, -1);
...
local_variable_found:
        ...
        if (variable) {
            ...
            if (variable->value != NULL) {
                ...
                XSLT_TRACE(tctxt,XSLT_TRACE_VARIABLES,xsltGenericDebug(xsltGenericDebugContext,
                    "found variable (local) '%s'\n", name));
                XSLT_TRACE(((xsltTransformContextPtr)ctxt),XSLT_TRACE_VARIABLES,xsltGenericDebug(xsltGenericDebugContext,
                    "XPathObjectType type [%u]\n", valueObj->type));
            }
    ...
    */
    ...
    if (valueObj == NULL) {
      XSLT_TRACE(tctxt,XSLT_TRACE_VARIABLES,xsltGenericDebug(xsltGenericDebugContext, "variable not found '%s'\n", name));

      if (ns_uri) xsltTransformError(tctxt, NULL, tctxt->inst, "Variable '{%s}%s' has not been declared.\n", ns_uri, name);
      else        xsltTransformError(tctxt, NULL, tctxt->inst, "Variable '%s' has not been declared.\n", name);
      /* xsltTransformError can return now, indicating that we should try to continue 
       * return an empty node-set as it is flexible to avoid further errors
       */
      valueObj = xmlXPathNewNodeSet(NULL, NULL);
    } else {
      XSLT_TRACE(tctxt,XSLT_TRACE_VARIABLES,xsltGenericDebug(xsltGenericDebugContext, "found variable (global) '%s'\n", name));
      XSLT_TRACE(((xsltTransformContextPtr)ctxt),XSLT_TRACE_VARIABLES,xsltGenericDebug(xsltGenericDebugContext, "XPathObjectType type [%u]\n", valueObj->type));
    }
