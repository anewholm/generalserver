 ...
 * </xsl:else>
 */
typedef struct _xsltStyleItemElse xsltStyleItemElse;
typedef xsltStyleItemElse *xsltStyleItemElsePtr;

struct _xsltStyleItemElse {
    XSLT_ITEM_COMMON_FIELDS
};
 ...
 * </xsl:else-if>
 */
typedef struct _xsltStyleItemElseIf xsltStyleItemElseIf;
typedef xsltStyleItemElseIf *xsltStyleItemElseIfPtr;

struct _xsltStyleItemElseIf {
    XSLT_ITEM_COMMON_FIELDS
...
struct _xsltStylesheet {
    ...
    void *templatesHash;  /* hash table or wherever compiled templates informations are stored */
    ...
    /* Annesley: new template types */
    void *allModesMatch; /* templates with @match=x and @mode=* */
...
#ifdef LIBXML_RR
    /* Annesley: extra vars */
    size_t usage_locks; /* number of process currently using this stylesheet. can be released at 0 */
    xmlGrammarProcessorCallbackContextPtr gp;
    xmlChar *adorn_literals; /* xsl:output adorn-literals=1 => xsl:is-literal */
#endif
};
...
struct _xsltTransformContext {
    ...
    xmlListPtr parent_route;    /* the current parent_route being processed */
    ...
    xmlListPtr initialContextParentRoute;
     ...
     */
    xmlNodeFilterCallbackContextPtr  xfilter;  /* additional security */
    xmlNodeTriggerCallbackContextPtr xtrigger; /* before and after read and write triggers */
    void *emo;                                 /* links through to emo C++ object for extensions */
    int topElemVisited;                        /* indicates if the transformation is flowing on from a parent */
    int lastIfResult;                          /* saved by xsltIf() for following xsltElse() */
    int adorn_literals;                        /* xsl:output adorn-literals => xsl:is-literal */
};
...
XSLTPUBFUN int XSLTCALL
      xsltHasLocks       (xsltStylesheetPtr style);
XSLTPUBFUN int XSLTCALL
      xsltLockStylesheet (xsltStylesheetPtr style);
XSLTPUBFUN int XSLTCALL
      xsltUnlockStylesheet (xsltStylesheetPtr style);
XSLTPUBFUN int XSLTCALL
      xsltFreeStylesheet  (xsltStylesheetPtr style);
XSLTPUBFUN int XSLTCALL
      xsltIsBlank    (xmlChar *str);
XSLTPUBFUN int XSLTCALL
      xsltCopyStackElemList  (xsltTransformContextPtr ctxt,
...
XSLTPUBFUN xsltStylesheetPtr XSLTCALL
      xsltParseStylesheetImportedDocContext(xmlDocPtr doc,
...
/* Annesley:
 * function exposed for imports.c 
 * to dynamically add <xsl:include> at runtime 
 * because of new xpath multiple includes
 */
XSLTPUBFUN xsltStyleItemIncludePtr XSLTCALL
