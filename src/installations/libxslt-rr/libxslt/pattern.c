 ...
 *
 * Annesley: ONLY used to resolve [complex predicates] XSLT_OP_PREDICATE
 ...
 */
static int
xsltTestCompMatchDirect(xsltTransformContextPtr ctxt, xsltCompMatchPtr comp, xmlNodePtr node, xmlNsPtr *nsList, int nsNr) {
 ...
 */
static int
xsltTestCompMatch(xsltTransformContextPtr ctxt, xsltCompMatchPtr comp, xmlNodePtr node, const xmlChar *mode, const xmlChar *modeURI) {
    int i;
    ...
    for (;i < comp->nbStep;i++) {
        ...
        switch (step->op) {
            ...
            case XSLT_OP_ELEM:
                ...
                /* 
                 * Annesley: full name test without predicates with DP Strategy (inheritance)
                 * XSL is not using xpath here to resolve the @match
                 * in the case of a simple QName comparison
                 * Namespace test first because Strategy (inheritance) cannot happen between different namespaces
                 * 
                 * inheritance:
                 *   @match should match *derived* elements also
                 * e.g.
                 *   <xsl:template @match=object:User => step = XSLT_OP_ELEM object namespace, User name
                 *   should match object:Person       => node = object:Person
                 */
                if ( ((node->ns == NULL) && (step->value2 == NULL))   /* no Namespace (NULL) */
                  || (xmlStrEqual(step->value2, node->ns->href) == 1) /* Namespace (href might be NULL) */
                ) {
                    /* local-name test */
                    if (xmlStrEqual(step->value, node->name) == 1) {
                        continue;
                    } else {
                        /* we have verifed that the namespace URIs match 
                         * so lets use the node prefix
                         * do not need parent_route
                         */
                        if (xmlXPathProcessingCall(ctxt->xpathCtxt->xprocessor, node, ctxt->xpathCtxt->parent_route, NODE_TEST_NAME, (node->ns ? node->ns->prefix : NULL), step->value, NULL) != 0)
                            continue;
                    }
                }
                goto rollback;
            ...
            case XSLT_OP_PREDICATE: {
                ...
                if (comp->direct) {
                    ...
                    return(xsltTestCompMatchDirect(ctxt, comp, node, comp->nsList, comp->nsNr));
                }
    ...
    if (CUR == '(') {
        ...
        if (CUR != ':') {
            ...
            if (xmlStrEqual(token, (const xmlChar *) "child")) {
                ...
                xsltTransformError(NULL, NULL, NULL,
                    "xsltCompileStepPattern : 'child' or 'attribute' expected (XSLT spec for performance reasons)\n");
                ctxt->error = 1;
                goto error;
            }
            xmlFree(token);
...
xsltCompilePatternInternal(const xmlChar *pattern, xmlDocPtr doc,
    ...
    if (pattern == NULL) {
        xsltTransformError(NULL, NULL, node, "xsltCompilePattern : NULL pattern\n");
        return(NULL);
    }
    ...
    while (pattern[current] != 0) {
        ...
        XSLT_TRACE_GENERIC1(XSLT_TRACE_PATTERN, "xsltCompilePattern : parsing '%s'",
                         element->pattern);
        /*
        ...
        XSLT_TRACE_GENERIC2(XSLT_TRACE_PATTERN, "xsltCompilePattern : parsed %s, default priority %f",
 ...
 */
int
xsltAddTemplate(xsltCompilerCtxtPtr cctxt, xsltTemplatePtr cur, const xmlChar *mode, const xmlChar *modeURI) {
    ...
    float priority;              /* the priority */
    xsltStylesheetPtr style = cctxt->style;
    
    /* DP Strategy for additional template fast lookup groupings */
    int additional_groupings;
    ...
    pat      = xsltCompilePatternInternal(cur->match, style->doc, cur->elem, style, NULL, 1);
    if (pat == NULL)
            return(-1);
    while (pat) {
        next = pat->next;
        ...
        */
        ...
        /*
         * Annesley: new @mode=* template
         * insert in to the @mode=* only if it is a generic mode catch
         * keep the name setting from the pattern above though
         */
        if ((pat->mode != NULL) && (pat->mode[0] == '*') && (pat->mode[1] == 0)) {
          top = &(style->allModesMatch);
        }
        ...
        if (name != NULL) {
            if (style->templatesHash == NULL) {
                ...
                list = (xsltCompMatchPtr) xmlHashLookup3(style->templatesHash, name, mode, modeURI);
                if (list == NULL) {
                    xmlHashAddEntry3(style->templatesHash, name, mode, modeURI, pat);
                } else {
                    if (list->priority <= pat->priority) {
                        pat->next = list;
                        xmlHashUpdateEntry3(style->templatesHash, name, mode, modeURI, pat, NULL);
                    } else {
            ...
            /*
             * XPathProcessingContext Strategy to suggest more names for this hash
             * @match CompTest is only done on name matching templates
             * i.e. Person => list of @match with that name, followed by Compiled xpath test
             *   which will probably only be a NODE_TEST_NAME anyway
             * only XSLT_REFACTORED_XPATHCOMP gives an xpath to the compiler
             */
            if ((cctxt->xpathCtxt != NULL) && (cctxt->xpathCtxt->xprocessor != NULL)) {
                additional_groupings = xmlXPathProcessingCall(cctxt->xpathCtxt->xprocessor, cur->elem, NULL, TEMPLATE_GROUP_NAMES, name, NULL, &groupings);
                while (additional_groupings) {
                    name = BAD_CAST groupings[--additional_groupings];
                    list = (xsltCompMatchPtr) xmlHashLookup3(style->templatesHash, name, mode, modeURI);
                    if (list == NULL) {
                        xmlHashAddEntry3(style->templatesHash, name, mode, modeURI, pat);
                    } else {
        ...
        if (mode) {
            XSLT_TRACE_GENERIC3(XSLT_TRACE_PATTERN, "added pattern : '%s' mode '%s' priority %f",
                             pat->pattern, pat->mode, pat->priority);
        } else {
            XSLT_TRACE_GENERIC2(XSLT_TRACE_PATTERN, "added pattern : '%s' priority %f",
 ...
 * 
 * Annesley: checks for new @mode=* where a mode template was not found
 *
 * Returns the xsltTemplatePtr or NULL if not found
 */
xsltTemplatePtr
xsltGetTemplate(xsltTransformContextPtr ctxt, xmlNodePtr node, xsltStylesheetPtr style)
{
    ...
    while ((curstyle != NULL) && (curstyle != style)) {
        ...
        if (name != NULL) {
             ...
             */
            if (xmlStrEqual(name, BAD_CAST "Manager"))
              list = NULL;
            list = (xsltCompMatchPtr) xmlHashLookup3(curstyle->templatesHash, name, ctxt->mode, ctxt->modeURI);
        } else
            list = NULL;
        while (list != NULL) {
            if (xsltTestCompMatch(ctxt, list, node, ctxt->mode, ctxt->modeURI)) {
                ret = list->template;
        ...
        /*
         * Annesley: new @mode=* generic mode matches
         * find the list of applicable expressions based on the name and the generic @mode=*
         * the @mode in allModesMatch is always *, so we test against itself
         * not the actual mode
         */
        if ((ret == NULL) && (ctxt->mode != NULL)) {
          list = curstyle->allModesMatch;
          while (list != NULL) {
              if (xsltTestCompMatch(ctxt, list, node, list->mode, NULL)) {
                  ret = list->template;
                  priority = list->priority;
                  break;
              }
...
xsltFreeTemplateHashes(xsltStylesheetPtr style) {
    ...
    if (style->allModesMatch != NULL)
        xsltFreeCompMatchList(style->allModesMatch);
}
