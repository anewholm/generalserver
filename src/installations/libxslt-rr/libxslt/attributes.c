...
xsltParseStylesheetAttributeSet(xsltStylesheetPtr style, xmlNodePtr cur) {
	...
	XSLT_TRACE_GENERIC(XSLT_TRACE_ATTRIBUTES, "creating attribute set table");
	...
	*/
	    ...
	    (! IS_XSLT_ELEM(child)))
	{
	    ...
	    */
	    ...
	    XSLT_TRACE_GENERIC1(XSLT_TRACE_ATTRIBUTES, "add attribute to list %s", ncname);
	    /*
...
next_child:
    ...
    */
	...
	while (*curval != 0) {
		...
		XSLT_TRACE_GENERIC2(XSLT_TRACE_ATTRIBUTES, "xsl:attribute-set : %s adds use %s", ncname, curval);
    ...
    */
    ...
    XSLT_TRACE_GENERIC1(XSLT_TRACE_ATTRIBUTES, "updated attribute list %s", ncname);
}
...
xsltResolveSASCallback(xsltAttrElemPtr values, xsltStylesheetPtr style,
	...
	if (tmp->set != NULL) {
     ...
     "xsl:attribute-set : use-attribute-sets recursion detected on %s\n",
		...
		XSLT_TRACE_GENERIC1(XSLT_TRACE_ATTRIBUTES, "Importing attribute list %s", tmp->set);
...
xsltMergeSASCallback(xsltAttrElemPtr values, xsltStylesheetPtr style,
	 ...
	 */
	XSLT_TRACE_GENERIC1(XSLT_TRACE_ATTRIBUTES, "attribute set %s present already in top stylesheet", name);
	...
	XSLT_TRACE_GENERIC1(XSLT_TRACE_ATTRIBUTES, "attribute set %s moved to top stylesheet", name);
    }
}
...
xsltResolveStylesheetAttributeSet(xsltStylesheetPtr style) {
    ...
    XSLT_TRACE_GENERIC(XSLT_TRACE_ATTRIBUTES, "Resolving attribute sets references");
    /*
                      ...
                      int fromAttributeSet)
{
    ...
    */
    ...
    if ((targetElem->type != XML_ELEMENT_NODE) && (targetElem->type != XML_PI_NODE))
	return;
    ...
    */
	...
	/* TODO: fix attr acquisition wrt to the XSLT namespace */
        prop = xsltEvalAttrValueTemplate(ctxt, inst, ctxt->xpathCtxt->parent_route, (const xmlChar *) "name", XSLT_NAMESPACE);
        if (prop == NULL) {
            xsltTransformError(ctxt, NULL, inst, "xsl:attribute: The attribute 'name' is missing.\n");
            goto error;
        }
	...
	*/
	if ((prefix != NULL) && (!xmlStrncasecmp(prefix, (xmlChar *) "xmlns", 5)))
	{
...
#ifdef LIBXML_RR
      xsltTransformError(ctxt, NULL, inst,
        "xsltAttributeInternal: xsl:attribute tried to create xmlns:ns_1 disallowed in GS "
        "namespace binding for the generated attribute '{%s}%s'.\n",
        nsName, name);
      goto error;
#endif
	    xmlChar *pref = xmlStrdup(BAD_CAST "ns_1");
	    ns = xsltGetSpecialNamespace(ctxt, inst, nsName, BAD_CAST pref, targetElem);
	    xmlFree(pref);
	} else {
	    ns = xsltGetSpecialNamespace(ctxt, inst, nsName, prefix, targetElem);
	}
	if (ns == NULL) {
	    xsltTransformError(ctxt, NULL, inst,
    ...
    */
    ...
    while (*curstr != 0) {
	    ...
	    */
            ...
            XSLT_TRACE_GENERIC1(XSLT_TRACE_ATTRIBUTES, "apply curstrute set %s", curstr);
