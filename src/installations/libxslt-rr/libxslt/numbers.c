				 ...
				 xmlNodePtr elem)
{
	...
	for (ancestor = node;
	     ...
	     ancestor = xmlXPathNextAncestor(parser, ancestor, 0)) {
		...
		for (preceding = ancestor;
		     preceding != NULL;
		     preceding =
		        xmlXPathNextPrecedingSibling(parser, preceding, 0)) {
		    if (count == NULL) {
		 ...
		 xmlNodePtr node)
{
	...
	data->format = xsltEvalAttrValueTemplate(ctxt, data->node, ctxt->xpathCtxt->parent_route,
