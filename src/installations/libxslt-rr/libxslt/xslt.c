...
xsltCompilationCtxtFree(xsltCompilerCtxtPtr cctxt)
{
    if (cctxt == NULL)
        return;
    XSLT_TRACE_GENERIC(XSLT_TRACE_PARSING, "Freeing compilation context");
    XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "### Max inodes: %d", cctxt->maxNodeInfos);
    XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "### Max LREs  : %d\n", cctxt->maxLREs);
 ...
 */
static xmlNodePtr xsltReadableContextNode(xsltCompilerCtxtPtr context, xmlNodePtr cur) {
  if ((context == NULL) || (cur == NULL)) return(NULL);
  return xmlReadableContextNode(context->xpathCtxt, cur);
}
static xmlNodePtr xsltReadableContextChildren(xsltCompilerCtxtPtr context, xmlNodePtr parent, int ignore_type_policy) {
  if ((context == NULL) || (parent == NULL)) return(NULL);
  return xmlReadableContextChildren(context->xpathCtxt, parent, ignore_type_policy);
}
/* NOT_CURRENTLY_USED
static xmlNodePtr xsltReadableContextNext(xsltCompilerCtxtPtr context, xmlNodePtr cur, int ignore_type_policy) {
  if ((context == NULL) || (cur == NULL)) return(NULL);
  return xmlReadableContextNext(context->xpathCtxt, cur, ignore_type_policy);
}
static xmlNodePtr xsltReadableContextPrev(xsltCompilerCtxtPtr context, xmlNodePtr cur, int ignore_type_policy) {
  if ((context == NULL) || (cur == NULL)) return(NULL);
  return xmlReadableContextPrev(context->xpathCtxt, cur, ignore_type_policy);
}
static xmlNodePtr xsltReadableContextParent(xsltCompilerCtxtPtr context, xmlNodePtr child, int ignore_type_policy) {
  if ((context == NULL) || (child == NULL)) return(NULL);
  return xmlReadableContextParent(context->xpathCtxt, child, ignore_type_policy);
}
static xmlNodePtr xsltReadableContextLast(xsltCompilerCtxtPtr context, xmlNodePtr parent, int ignore_type_policy) {
  if ((context == NULL) || (parent == NULL)) return(NULL);
  return xmlReadableContextLast(context->xpathCtxt, parent, ignore_type_policy);
}
static xmlNodePtr xsltReadableContextDocRoot(xsltCompilerCtxtPtr context, xmlNodePtr doc) {
...
xsltCompilationCtxtCreate(xsltStylesheetPtr style, xmlXPathContextPtr xctxt) {
    xsltCompilerCtxtPtr ret;

    if (xctxt == NULL) {
        xsltTransformError(NULL, style, NULL,
            "xsltCompilationCtxtCreate: need an xpath context.\n");
        return(NULL);
    }
...
xsltNewStylesheet(void) {
    ...
    XSLT_TRACE_GENERIC_ANY("creating dictionary for stylesheet");

#ifdef LIBXML_RR
    ret->usage_locks = 0;
    ret->gp = NULL;
    ret->adorn_literals = NULL;
#endif
 ...
 * @style:  an XSLT stylesheet
 */
int xsltLockStylesheet(xsltStylesheetPtr style) {
  if (style == NULL) return 0;
  return ++style->usage_locks;
}
/**
 * @author: Annesley
 * xsltHasLocks:
 * @style:  an XSLT stylesheet
 */
int xsltHasLocks(xsltStylesheetPtr style) {
  if (style == NULL) return 0;
  return style->usage_locks;
}
/**
 * @author: Annesley
 * xsltUnlockStylesheet:
 * @style:  an XSLT stylesheet
 */
int xsltUnlockStylesheet(xsltStylesheetPtr style) {
  if (style == NULL) return 0;
  return style->usage_locks ? --style->usage_locks : 0;
}
...
{
    /* Annesley: only free sheets that are not in use */
    if (style->usage_locks != 0) return 1;
    ...
    */
    ...
    XSLT_TRACE_GENERIC_ANY("freeing dictionary from stylesheet");
...
skip_ns:
    ...
    if (nbns != 0) {
        XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "template has %d inherited namespaces", nbns);
...
#ifdef LIBXML_RR
    prop = xmlGetNsProp(cur, (const xmlChar *) "adorn-literals", NULL);
    if (prop != NULL) {
    ...
    */
        ...
        */
        ...
        if (prop == NULL) {
            ...
            if ((xmlStrEqual(prop, (const xmlChar *) "xml")) ||
                ...
                xsltTransformError(NULL, style, cur, "invalid value for method: %s\n", prop);
    ...
    if (elements != NULL) {
        ...
        while (*element != 0) {
            ...
            if (element) {
                XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "add cdata section output element %s", element);
...
xsltParseStylesheetPreserveSpace(xsltStylesheetPtr style, xmlNodePtr cur) {
    ...
    while (*element != 0) {
        ...
        element = xmlStrndup(element, end - element);
        if (element) {
            XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "add preserved space element %s\n", element);
    ...
    while (*prefix != 0) {
        ...
        if (prefix) {
            ...
            "xsl:extension-element-prefix : undefined namespace %s\n",
                ...
                XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "add extension prefix %s", prefix);
...
xsltParseStylesheetStripSpace(xsltStylesheetPtr style, xmlNodePtr cur) {
    ...
    while (*element != 0) {
        ...
        element = xmlStrndup(element, end - element);
        if (element) {
            XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "add stripped space element %s", element);
            if (xmlStrEqual(element, (const xmlChar *)"*")) {
                style->stripAll = 1;
            } else {
                const xmlChar *URI;
        ...
        if (prefix) {
            ...
            "xsl:exclude-result-prefixes : undefined namespace %s\n",
                ...
                if (exclPrefixPush(style, (xmlChar *) ns->href) >= 0) {
                    XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "exclude result prefix %s", prefix);
                    nb++;
                }
            }
            xmlFree(prefix);
        ...
        */
        ...
        if (ns == NULL) {
            ...
            */
            ...
            XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "resolved prefix '%s'", cur);
            /*
...
{
    xmlNodePtr deleteNode, cur, next, txt, textNode = NULL;
    xmlDocPtr doc;
    xsltStylesheetPtr style;
    int internalize = 0, /* Annesley: ensure all strings are in style->dict == doc->dict */
        findSpaceAttr;
    int xsltStylesheetElemDepth;
    int sharedDoc; /* Annesley */
    xmlAttrPtr attr;
    xmlChar *value;
    const xmlChar *name;
    int strictWhitespace, inXSLText = 0;
#ifdef XSLT_REFACTORED_XSLT_NSCOMP
    const xmlChar *nsNameXSLT = NULL;
    ...
    if ((style->dict != NULL) && (doc->dict == style->dict))
        /* Annesley: ensure all strings are in style->dict == doc->dict */
        internalize = 1;
    else
    ...
    */
    if (! cctxt->simplified) xsltStylesheetElemDepth = cctxt->depth +1;
    else                     xsltStylesheetElemDepth = 0;

    if (xmlNodeGetSpacePreserve(node) != 1) cctxt->inode->preserveWhitespace = 0;
    else                                    cctxt->inode->preserveWhitespace = 1;
    ...
    */
    ...
    /* Annesley: sharedDoc optimization
     * this xmlDocPtr has a root node in another document
     * so we do not change the other document
     * or its DICT or settings
     *  XML_TEXT_NODEs will not be merged 
     *  XML_PI_NODEs   will not be removed 
     * Do this in the original doc instead
     * 
     * xsl:template dynamic @match can reference the stylesheet's original parents
     */
    sharedDoc  = xmlIsSharedDoc(cctxt->style->doc);
    deleteNode = NULL;
    cur        = node;
    while (cur != NULL) {
        if ((deleteNode != NULL) && (sharedDoc == 0)) {
            XSLT_TRACE_GENERIC(XSLT_TRACE_BLANKS, "xsltParsePreprocessStylesheetTree: removing node");
...
process_attributes:
                ...
                do {
                    ...
                    if (internalize) {
                        ...
                        */
                        ...
                        {
                            value = (xmlChar *) xmlDictLookup(style->dict, txt->content, -1);
            ...
            */
            next = xsltReadableContextChildren(cctxt, cur, SKIP_DTD_AND_ENTITY);
            if (next != NULL) {
                cur = next;
                continue;
            }
        ...
        if (textNode) {
            ...
            */
                ...
                (!xmlDictOwns(style->dict, textNode->content)))
            {
                ...
                */
                value = (xmlChar *) xmlDictLookup(style->dict, textNode->content, -1);
...
next_sibling:
        ...
        /* Annesley: remember node = xsl:stylesheet 
         * in the same document with correct node->children->parent
         * NOT the XML_DOCUMENT which might be shared
         */
        if (cur == node)
            break;
        if (cur->next != NULL) {
    ...
    if ((deleteNode != NULL) && (sharedDoc == 0)) {
        XSLT_TRACE_GENERIC(XSLT_TRACE_PARSING, "xsltParsePreprocessStylesheetTree: removing node");
        xmlUnlinkNode(deleteNode);
        xmlFreeNode(deleteNode);
    }
    return(0);
        ...
        if (cur->type == XML_ELEMENT_NODE) {
            ...
            while (ns != NULL) {
             ...
             "Namespaces prefix %s used for multiple namespaces\n",ns->prefix);
                        ...
                        XSLT_TRACE_GENERIC2(XSLT_TRACE_PARSING, "Added namespace: %s mapped to %s", ns->prefix, ns->href);
                    }
                }
                ns = ns->next;
            }
                             ...
                             xmlNodePtr node)
{
    ...
    if (node->name[0] == 'a') {
        ...
        if (IS_XSLT_NAME(node, "else"))
            return(XSLT_FUNC_ELSE);
        if (IS_XSLT_NAME(node, "else-if"))
            return(XSLT_FUNC_ELSEIF);
    ...
    */
    switch (cctxt->inode->type) {
        ...
        case XSLT_FUNC_WITHPARAM:
            ...
            goto sequence_constructor;
        default:
            XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "xsltParseXSLTNode: Unhandled XSLT element '%s'.", elem->name);
            xsltTransformError(NULL, cctxt->style, elem,
    ...
    */
    while (cur != NULL) {
        if (deleteNode != NULL)        {
            ...
            XSLT_TRACE_GENERIC(XSLT_TRACE_BLANKS, "xsltParseSequenceConstructor: removing xsl:text element");
            xmlUnlinkNode(deleteNode);
            xmlFreeNode(deleteNode);
            deleteNode = NULL;
        }
                    ...
                    (xsltCheckExtPrefix(cctxt->style, cur->ns->href) == 1))
                {
                    ...
                    */
                    ...
                    if (cur->properties != NULL) {
                        ...
                        * 
                        * Annesley: we stamp xsl:is-literal on all outgoing literals now
                        */
                        do {
                            if ((attr->psvi != xsltXSLTAttrMarker) && IS_XSLT_ATTR_FAST(attr)) {
                                if ( (xmlStrEqual(attr->name, BAD_CAST "use-attribute-sets") == 0)
                                  && (xmlStrEqual(attr->name, BAD_CAST "is-literal") == 0) /* Annesley: we stamp xsl:is-literal on all outgoing literals now */
                                ) {
                                    xsltTransformError(NULL, cctxt->style,
                                        cur,
...
xsltParseStylesheetKey(xsltStylesheetPtr style, xmlNodePtr key) {
    ...
    if (prop != NULL) {
        ...
        */
        ...
        XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "xsltParseStylesheetKey: name %s", name);
    } else {
...
xsltParseXSLTTemplate(xsltCompilerCtxtPtr cctxt, xmlNodePtr templNode) {
    ...
    /*
    * Attribute "with-all-params".
    */
    prop = xmlGetNsProp(templNode, (const xmlChar *)"with-all-params", NULL);
    if (prop != NULL) {
        if (xmlStrEqual(prop, BAD_CAST "all")) {
          templ->withAllParams = 1;
          XSLT_TRACE_GENERIC(XSLT_TRACE_PARSING, "xsltParseXSLTTemplate: with-all-params on");
        }
        xmlFree(prop);
    }
    ...
    */
    ...
    if (prop != NULL) {
        ...
        */
        ...
        xmlFree(prop);
        prop = NULL;
        /* Annesley: new @mode=* low priority catch template */
        if (xmlValidateNCName(templ->mode, 0) && (!xmlStrEqual(templ->mode, BAD_CAST "*"))) {
        ...
        XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "xsltParseXSLTTemplate: mode %s", templ->mode);
    }
    /*
    * Attribute "match".
    * Annesley: dynamic match values
    * this allows matches to respect inherited elements
    * e.g. @match={database:derived-template-match()}
    */
    prop = xmlGetNsProp(templNode, (const xmlChar *)"match", NULL);
    if (prop != NULL) {
        if (xmlStrchr(prop, '{')) {
          if (cctxt->xpathCtxt->externalContext == NULL) {
            /* xsl:template @match is dynamic during compilation stage
             * but no parent transform (with context node) available
             * therefore, use the stylesheet doc as the xpath context doc
             */
            tCtxt = (xsltTransformContextPtr) xsltNewTransformContext(cctxt->style, cctxt->style->doc);
          } else {
            /* parent transform taking place: use that context */
            tCtxt = (xsltTransformContextPtr) cctxt->xpathCtxt->externalContext;
          }
          oldprop = prop;
          oldnode = tCtxt->node;
          cctxt->xpathCtxt->node = templNode;
          tCtxt->node            = templNode;
          /* Annesley: NULL: currently we do not honour hardlinks and deviations in template code */
          prop    = xsltAttrTemplateValueProcessNode(tCtxt, oldprop, templNode, NULL);
          tCtxt->node            = oldnode;
          cctxt->xpathCtxt->node = oldnode;
          
          /* free up */
          xmlFree(oldprop);
          if (cctxt->xpathCtxt->externalContext == NULL) xsltFreeTransformContext(tCtxt);
        }
        if (prop != NULL) {
            templ->match  = prop;
            prop = NULL;
    ...
    */
    ...
    xsltAddTemplate(cctxt, templ, templ->mode, templ->modeURI);
    ...
    */
    ...
    if (prop != NULL) {
        ...
        */
        ...
        XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "xsltParseStylesheetTemplate: mode %s", mode);
...
xsltParseXSLTStylesheetElemCore(xsltCompilerCtxtPtr cctxt, xmlNodePtr node)
{
    ...
    */
    cur = node->children;
    while ((cur != NULL) && xsltParseFindTopLevelElem(cctxt, cur, BAD_CAST "import", XSLT_NAMESPACE, 1, &cur) == 1)
        cur = cur->next;
    if (cur == NULL)
        goto exit;
    ...
    */
    while ((cur != NULL) && xsltParseFindTopLevelElem(cctxt, cur, BAD_CAST "param", XSLT_NAMESPACE, 0, &cur) == 1)
    {
        /* Annesley: CompilationEnvironment triggers and changes only */
        xsltReadableContextNode(cctxt, cur);
    ...
    */
    cur = start;
    while ((cur != NULL) && xsltParseFindTopLevelElem(cctxt, cur, BAD_CAST "variable", XSLT_NAMESPACE, 0, &cur) == 1)
    {
        /* Annesley: CompilationEnvironment triggers and changes only */
        xsltReadableContextNode(cctxt, cur);
    ...
    */
    ...
    while (cur != NULL) {
        ...
        */
        if (cur->type == XML_ELEMENT_NODE) {
            if (cur->ns == NULL) {
                xsltTransformError(NULL, style, cur, "Unexpected top-level element in no namespace.\n");
                style->errors++;
                cur = cur->next;
                continue;
            }
            ...
            if (IS_XSLT_ELEM_FAST(cur)) {
                ...
                */
                if (IS_XSLT_NAME(cur, "import")) {
                    xsltTransformError(NULL, style, cur, "Misplaced xsl:import element.\n");
                    style->errors++;
                    cur = cur->next;
                    continue;
                }
                ...
                */
                ...
                if (function != NULL)
                    function(style, cur);
                XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING,
                    "xsltParseXSLTStylesheetElemCore : User-defined "
                    "data element '%s'.", cur->name);
            }
        }
        /* CompilationContext xsltReadableContextNode(cctxt, cur) at start */
        cur = cur->next;
    }
...
exit:
    ...
    XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "### END of parsing top-level elements of doc '%s'.", node->doc->URL);
    XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "### Templates: %d\n", templates);
#ifdef XSLT_REFACTORED
    XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "### Max inodes: %d\n", cctxt->maxNodeInfos);
    XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "### Max LREs  : %d\n", cctxt->maxLREs);
#endif /* XSLT_REFACTORED */
...
xsltParseXSLTStylesheetElem(xsltCompilerCtxtPtr cctxt, xmlNodePtr node)
{
    ...
    */
    cur = start;
    while ((cur != NULL) && xsltParseFindTopLevelElem(cctxt, cur, BAD_CAST "include", XSLT_NAMESPACE, 0, &cur) == 1)
    {
    ...
    */
    cur = start;
    while ((cur != NULL) &&
        xsltParseFindTopLevelElem(cctxt, cur, BAD_CAST "namespace-alias", XSLT_NAMESPACE, 0, &cur) == 1)
    {
...
xsltParseStylesheetTop(xsltStylesheetPtr style, xmlNodePtr top) {
    ...
    while (cur != NULL) {
        ...
        if ((cur->type == XML_ELEMENT_NODE) && (!(IS_XSLT_ELEM(cur)))) {
            ...
            XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "xsltParseStylesheetTop : found foreign element %s", cur->name);
            cur = cur->next;
            continue;
        }
        if (IS_XSLT_NAME(cur, "import")) {
    ...
    XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "parsed %d templates", templates);
}
                                  ...
                                  xmlNodePtr node)
{
    ...
    XSLT_TRACE_GENERIC(XSLT_TRACE_PARSING, "xsltParseSimplifiedStylesheetTree: document is stylesheet");
    ...
    */
    ...
    templ->content = node;
    xsltAddTemplate(cctxt, templ, NULL, NULL);
...
xsltParseStylesheetProcess(xsltStylesheetPtr style, xmlDocPtr doc)
{
    ...
    if ((IS_XSLT_ELEM(cur)) &&
        ...
        XSLT_TRACE_GENERIC(XSLT_TRACE_PARSING, "xsltParseStylesheetProcess : found stylesheet");
        cctxt->simplified = 0;
...
xsltParseStylesheetProcess(xsltStylesheetPtr ret, xmlDocPtr doc) {
    ...
    if (ret->literal_result == 0) {
        ...
        if (prop == NULL) {
            xsltTransformError(NULL, ret, cur, "xsltParseStylesheetProcess : document is not a stylesheet\n");
            return(NULL);
        }

        XSLT_TRACE_GENERIC(XSLT_TRACE_PARSING, "xsltParseStylesheetProcess : document is stylesheet");

        if (!xmlStrEqual(prop, (const xmlChar *)"1.0")) {
            xsltTransformError(NULL, ret, cur, "xsl:version: only 1.0 features are supported\n");
             /* TODO set up compatibility when not XSLT 1.0 */
            ret->warnings++;
        }
        xmlFree(prop);
 ...
 */
xsltStylesheetPtr
xsltParseStylesheetImportedDoc(xmlDocPtr doc, xsltStylesheetPtr parentStyle) {
  return xsltParseStylesheetImportedDocContext(doc, parentStyle, NULL);
}

xsltStylesheetPtr
xsltParseStylesheetImportedDocContext(xmlDocPtr doc, xsltStylesheetPtr parentStyle, xmlXPathContextPtr xctxt) {
    ...
    */
    if (doc->dict != NULL) {
        ...
        XSLT_TRACE_GENERIC_ANY1("reusing dictionary from %s for stylesheet", doc->URL);
    ...
    */
...
#ifdef XSLT_REFACTORED
    {
        ...
        if (parentStyle == NULL) {
            ...
            */
            cctxt = xsltCompilationCtxtCreate(retStyle, xctxt);
            if (cctxt == NULL) {
                xsltFreeStylesheet(retStyle);
                return(NULL);
            }
...
xsltParseStylesheetFile(const xmlChar* filename) {
    ...
    XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "xsltParseStylesheetFile : parse %s", filename);
...
xsltLoadStylesheetPI(xmlDocPtr doc) {
     ...
     */
    if (href != NULL) {
        XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "xsltLoadStylesheetPI : found PI href=%s", href);
        ...
        if ((URI->fragment != NULL) && (URI->scheme == NULL) &&
            ...
            XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "xsltLoadStylesheetPI : Reference to ID %s", href);
            if (URI->fragment[0] == '#')
            ...
            if (ID == NULL) {
                ...
                XSLT_TRACE_GENERIC_ANY1("creating new document from %s for embedded stylesheet", doc->URL);
                ...
                if (fake != NULL) {
                    ...
                    */
                    ...
                    XSLT_TRACE_GENERIC_ANY1("reusing dictionary from %s for embedded stylesheet", doc->URL);

                    newtree = xmlDocCopyNode(subtree, fake, 1);

                    fake->URL = xmlNodeGetBase(doc, subtree->parent);
                    XSLT_TRACE_GENERIC_ANY1("set base URI for embedded stylesheet as %s", fake->URL);
            ...
            if (URL != NULL) {
                XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "xsltLoadStylesheetPI : fetching %s", URL);
                ret = xsltParseStylesheetFile(URL);
                xmlFree(URL);
            } else {
                XSLT_TRACE_GENERIC1(XSLT_TRACE_PARSING, "xsltLoadStylesheetPI : fetching %s", href);
