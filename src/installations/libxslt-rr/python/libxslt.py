...
class transformCtxt(transformCtxtBase):
    ...
    def printErrorContext(self, style, node, msg):
        """Display the context of an error."""
        if style == None: style__o = None
        else: style__o = style._o
        if node == None: node__o = None
        else: node__o = node._o
        libxsltmod.xsltPrintErrorContext(self._o, style__o, node__o, msg)
...
class stylesheet(stylesheetBase):
    ...
    def applyStylesheetUser(self, doc, params, nodesets, output, profile, transformCtxt):
        """Apply the stylesheet to the document"""
        if doc == None: doc__o = None
        else: doc__o = doc._o
        if transformCtxt == None: transformCtxt__o = None
        else: transformCtxt__o = transformCtxt._o
        ret = libxsltmod.xsltApplyStylesheetUser(self._o, doc__o, params, nodesets, output, profile, transformCtxt__o)
        if ret == None: return None
        return libxml2.xmlDoc(_obj=ret)
    ...
    def printErrorContext(self, ctxt, node, msg):
        """Display the context of an error."""
        if ctxt == None: ctxt__o = None
        else: ctxt__o = ctxt._o
        if node == None: node__o = None
        else: node__o = node._o
        libxsltmod.xsltPrintErrorContext(ctxt__o, self._o, node__o, msg)
