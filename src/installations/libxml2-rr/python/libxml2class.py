...
class xmlDoc(xmlNode):
    ...
    def nodeDumpOutput(self, buf, cur, level, format, encoding, options):
        ...
        libxml2mod.xmlNodeDumpOutput(buf__o, self._o, cur__o, level, format, encoding, options)
...
class outputBuffer(ioWriteWrapper):
    ...
    def nodeDumpOutput(self, doc, cur, level, format, encoding, options):
        """Dump an XML node, recursive behaviour, children are printed
          too. Note that @format = 1 provide node indenting only if
          xmlIndentTreeOutput = 1 or xmlKeepBlanksDefault(0) was
           called """
        ...
        libxml2mod.xmlNodeDumpOutput(self._o, doc__o, cur__o, level, format, encoding, options)
