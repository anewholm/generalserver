...
struct _xmlSaveCtxt {
    ...
    xmlNodeFilterCallbackContextPtr xfilter;   /* Annesley: additional security context */
    xmlNodeTriggerCallbackContextPtr xtrigger; /* Annesley: additional trigger context */
    int maxdepth;                              /* Annesley: additional depth to allow printing only the element */
};

#include "xmlsave_gs.c"
...
xmlAttrDumpOutput(xmlSaveCtxtPtr ctxt, xmlAttrPtr cur) {
    ...
    /* Annesley: security checking
     * we are converting to (xmlNodePtr) because we need to check attributes
     * for Repository save without certain attributes
     */
    if (xmlReadableNode(ctxt->xfilter, ctxt->xtrigger, (xmlNodePtr) cur, NULL) == NULL) return;
...
xmlNodeDumpOutputInternal(xmlSaveCtxtPtr ctxt, xmlNodePtr cur) {
    ...
    xmlChar *start, *end;
    xmlOutputBufferPtr buf;
    int bShowChildren; /* Annesley: maxdepth calcs */

    if (cur == NULL) return;

#ifdef LIBXML_RR
    /* Annesley: maxdepth calcs */
    bShowChildren = (ctxt->maxdepth == 0 || ctxt->level + 1 < ctxt->maxdepth);
    /* Annesley: security checking */
    if (xmlReadableNode(ctxt->xfilter, ctxt->xtrigger, (xmlNodePtr) cur, NULL) == NULL) return;
#endif
    ...
    if (cur->type == XML_TEXT_NODE) {
        if (cur->content != NULL) {
            if ((cur->name == xmlStringTextNoenc) 
              || (ctxt->options & XML_SAVE_NOESCAPE)
            ) {
                /*
...
#ifdef LIBXML_RR
    /* Annesley: hardlink handling */
    if ((xmlIsHardLink(cur) == 1)) {
        xmlHardLinkDumpOutput(ctxt, cur);
        return;
    }
#endif
    ...
    if (((cur->type == XML_ELEMENT_NODE) || (cur->content == NULL)) &&
        (cur->children == NULL || bShowChildren == 0) && ((ctxt->options & XML_SAVE_NO_EMPTY) == 0)) {
        if (ctxt->format == 2)
            xmlOutputBufferWriteWSNonSig(ctxt, 0);
        xmlOutputBufferWrite(buf, 2, "/>");
        ctxt->format = format;
    ...
    if (cur->children != NULL) {
        ...
        if (bShowChildren == 1) xmlNodeListDumpOutput(ctxt, cur->children);
        if (ctxt->level > 0) ctxt->level--;
    ...
    if (((cur->type == XML_HTML_DOCUMENT_NODE) &&
        ...
        if (cur->children != NULL) {
            ...
            while (child != NULL) {
                ...
                /* Annesley: we are outputting multi-root node documents including text now 
                 * was unconditional which placed a newline after the HTTP Headers
                 * now, we output the last one as it will be truncated
                 * and we also continue to allow the unconditional formtting of the top nodes
                 * in future this could be controlled by:
                 *   ctxt->format == 1
                 */
                if ((child->type != XML_TEXT_NODE) || (child->next == NULL))
                  xmlOutputBufferWrite(buf, 1, "\n");
                child = child->next;
            }
        }
 ...
 * @maxdepth: limit output to a certain tree depth
 * @options:  extra save context options
 *
 * Dump an XML node, recursive behaviour,children are printed too.
 * Note that @format = 1 provide node indenting only if xmlIndentTreeOutput = 1
 * or xmlKeepBlanksDefault(0) was called
 ...
 */
int
xmlNodeDump(xmlBufferPtr buf, xmlDocPtr doc, xmlNodePtr cur, int level, int format, int options) {
  /* use xmlDebugDumpNodeXML(node) in GDB situations! */
  return xmlNodeDumpSecure(buf, doc, cur, level, format, 0, 0, 0, options);
}

int
xmlNodeDumpSecure(xmlBufferPtr buf, xmlDocPtr doc, xmlNodePtr cur, int level,
            int format, const xmlNodeTriggerCallbackContextPtr xtrigger, const xmlNodeFilterCallbackContextPtr xfilter, int maxdepth, int options)
{
    /* use xmlDebugDumpNodeXML(node) in GDB situations! */
    unsigned int use;
    int ret;
    ...
    use = buf->use;
    xmlNodeDumpOutputSecure(outbuf, doc, cur, level, format, NULL, xtrigger, xfilter, maxdepth, options);
    xmlFree(outbuf);
    ret = buf->use - use;
    return (ret);
}
...
xmlElemDump(FILE * f, xmlDocPtr doc, xmlNodePtr cur)
{
    ...
    if ((doc != NULL) && (doc->type == XML_HTML_DOCUMENT_NODE)) {
        ...
        xmlNodeDumpOutput(outbuf, doc, cur, 0, 1, NULL, 0);
 ...
 */
void 
xmlNodeDumpOutput(xmlOutputBufferPtr buf, xmlDocPtr doc, xmlNodePtr cur, int level, int format, const char *encoding, int options) {
  xmlNodeDumpOutputSecure(buf, doc, cur, level, format, encoding, 0, 0, 0, options);
}

void
xmlNodeDumpOutputSecure(xmlOutputBufferPtr buf, xmlDocPtr doc, xmlNodePtr cur,
                  int level, int format, const char *encoding, const xmlNodeTriggerCallbackContextPtr xtrigger, const xmlNodeFilterCallbackContextPtr xfilter, int maxdepth, int options)
{
    xmlSaveCtxt ctxt;
#ifdef LIBXML_HTML_ENABLED
    xmlDtdPtr dtd;
    ...
    ctxt.options |= options | XML_SAVE_AS_XML;
    ctxt.xtrigger = xtrigger;
    ctxt.xfilter = xfilter;
    ctxt.maxdepth = maxdepth;
 ...
 */
int
xmlSaveFormatFileEnc( const char * filename, xmlDocPtr cur, const char * encoding, int format ) {
    xmlSaveCtxt ctxt;
    xmlOutputBufferPtr buf;
    xmlCharEncodingHandlerPtr handler = NULL;
    int ret;
