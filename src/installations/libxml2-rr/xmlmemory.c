...
xmlMallocAtomicLoc(size_t size, const char * file, int line)
{
    ...
    if (xmlMemTraceBlockAt == ret) {
	xmlGenericError(xmlGenericErrorContext, "%p : Malloc(%zd) Ok\n", xmlMemTraceBlockAt, size);
	xmlMallocBreakpoint();
    }
...
xmlReallocLoc(void *ptr,size_t size, const char * file, int line)
{
	...
	xmlGenericError(xmlGenericErrorContext, "%p : Realloced(%zd -> %zd) Ok\n", xmlMemTraceBlockAt, p->mh_size, size);
	xmlMallocBreakpoint();
    }
    p->mh_tag = MEMTAG;
    p->mh_number = number;
...
#endif
     /* Annesley: moved after as we are using our own throw error handlers */
     xmlGenericError(xmlGenericErrorContext, "Memory tag error occurs :%p \n\t bye\n", p);
}
