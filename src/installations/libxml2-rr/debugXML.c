...
static void
xmlDebugErr4(xmlDebugCtxtPtr ctxt, int error, const char *msg, const char *extra1, const char *extra2)
{
    ctxt->errors++;
    __xmlRaiseError(NULL, NULL, NULL,
            NULL, ctxt->node, XML_FROM_CHECK,
...
{
    int ret;
    /* Annesley: better reporting: path to node causing problems */
    const char *path = 0;

    ret = xmlNsCheckScope(node, ns);

    if (ret < 0) {
      if (node) path = (const char*) xmlGetNodePath(node);
      if (ret == -2) {
          if (ns->prefix == NULL)
          xmlDebugErr3(ctxt, XML_CHECK_NS_SCOPE,
              "Reference to default namespace not in scope on \n  [%s]\n", path);
      else
          xmlDebugErr4(ctxt, XML_CHECK_NS_SCOPE,
              "Reference to namespace '%s' not in scope on \n  [%s]\n",
              (char *) ns->prefix, path);
      }
      if (ret == -3) {
          if (ns->prefix == NULL)
          xmlDebugErr3(ctxt, XML_CHECK_NS_ANCESTOR,
              "Reference to default namespace not on ancestor on \n  [%s]\n", path);
      else
          xmlDebugErr4(ctxt, XML_CHECK_NS_ANCESTOR,
              "Reference to namespace '%s' not on ancestor on \n  [%s]\n",
              (char *) ns->prefix, path);
      }
    }
}
...
{
    /* Annesley: better reporting */
    const xmlChar *sPath = 0;
    if (ctxt && ctxt->node) sPath = xmlGetNodePath(ctxt->node);
    ...
    if (ctxt->check) {
      if (name == NULL) {
          ...
          xmlDebugErr4(ctxt, XML_CHECK_NOT_NCNAME,
              "Name is not an NCName '%s' on [%s]",
              (const char *) name,
              (const char *) sPath);
      }
      if ((ctxt->dict != NULL) &&
          ...
          xmlDebugErr4(ctxt, XML_CHECK_OUTSIDE_DICT,
              "Name is not from the document dictionnary '%s' on [%s]",
              (const char *) name,
              (const char *) sPath);
      }
    }

    if (sPath) xmlFree((void*) sPath);
}
...
xmlCtxtGenericNodeCheck(xmlDebugCtxtPtr ctxt, xmlNodePtr node) {
    ...
    /* Annesley: added check for DOC node which should not have a parent */
    if ((node->parent == NULL) && (node->type != XML_DOCUMENT_NODE)) 
    ...
    if (node->doc == NULL) {
      ...
      dict = doc->dict;
      if ((dict == NULL) && (ctxt->nodict == 0)) ctxt->nodict = 1;
      if (ctxt->doc == NULL)  ctxt->doc = doc;
      if (ctxt->dict == NULL) ctxt->dict = dict;
    }
    ...
    if (node->type == XML_ELEMENT_NODE) {
        ...
        if (node->ns != NULL) xmlCtxtNsCheckScope(ctxt, node, node->ns);
    ...
    if ((node->type != XML_ELEMENT_NODE) &&
      ...
      if (node->content != NULL)
        xmlCtxtCheckString(ctxt, (const xmlChar *) node->content); /* UTF-8 check only */
    }
    ...
    switch (node->type) {
        ...
        case XML_ATTRIBUTE_NODE:
          ...
          if ((dict != NULL) && (xmlDictOwns(dict, node->name) == 0)) 
            xmlDebugErr3(ctxt, XML_CHECK_WRONG_NAME, "Text name not in dictionary '%s'", (const char *) node->name);
          /*
          if ( (dict != NULL) 
            && (node->children != NULL) 
            && (node->children->content != NULL) 
            && (xmlDictOwns(dict, node->children->content) == 0)) 
            xmlDebugErr3(ctxt, XML_CHECK_WRONG_NAME, "Attribute content not in dictionary '%s'", (const char *) node->name);
          */
          break;
        case XML_TEXT_NODE:
          if ((node->name == xmlStringText) || (node->name == xmlStringTextNoenc)) break;
          /* some case of entity substitution can lead to this */
          if ((ctxt->dict != NULL) && (node->name == xmlDictLookup(ctxt->dict, BAD_CAST "nbktext", 7))) break;
          xmlDebugErr3(ctxt, XML_CHECK_WRONG_NAME, "Text node has wrong name '%s'", (const char *) node->name);
          break;
        case XML_COMMENT_NODE:
          if (node->name == xmlStringComment) break;
...
/**
 * xmlNodeListOutputWalker:
 * @node_list:  the FILE * for the output
 *
 * @author: Annesley
 * 
 * Dumps debug information for the node list
 */
void *
xmlNodeListOutputWalker(const void *node, const void *user1 ATTRIBUTE_UNUSED, const void *user2 ATTRIBUTE_UNUSED) {
  xmlDebugDumpOneNode(stderr, (xmlNodePtr) node, 0, 0);
  return (NULL);
}
 ...
 * @output:  the FILE * for the output
 * @node:  the node
 * @include_attributes: traverse and dump attributes also
 *
 * Dumps debug information for the element node, it is not recursive
 */
static void
xmlCtxtDumpOneNode(xmlDebugCtxtPtr ctxt, xmlNodePtr node, int include_attributes)
{
    xmlChar *xml_id;
    ...
    switch (node->type) {
        case XML_ELEMENT_NODE:
            if (!ctxt->check) {
                ...
                /* Annesley: hardlink? */
                if ((xmlIsHardLinked(node) == 1)) {
                  fprintf(ctxt->output, "(hardlink");
                  if (xmlIsOriginalHardLink(node) == 1) fprintf(ctxt->output, "ed");
                  fprintf(ctxt->output, ") ");
                }
                
                /* Annesley: deviations? */
                if (xmlHasDeviants( node, NULL) == 1) fprintf(ctxt->output, "+deviants ");
                if (xmlHasOriginals(node, NULL) == 1) fprintf(ctxt->output, "+originals ");
                ...
                /* Annesley: (@xml:id) */
                xml_id = xmlGetNsProp(node, (const xmlChar*) "id", XML_XML_NAMESPACE);
                if (xml_id != NULL) {
                  fprintf(ctxt->output, " (%s)", xml_id);
                  xmlFree(xml_id);
                }
...
xmlCtxtDumpNode(xmlDebugCtxtPtr ctxt, xmlNodePtr node)
{
    ...
    xmlCtxtDumpOneNode(ctxt, node, 1);
    if ((node->type != XML_NAMESPACE_DECL) &&
        (node->children != NULL) &&
        (node->type != XML_ENTITY_REF_NODE) &&
        (xmlIsHardLink(node) == 0)  /* Annesley: special hardlink handling */
    ) {
        ctxt->depth++;
 ...
 * @author: Annesley
 *
 * Dumps XML for the element node, it is recursive
 */
xmlChar*
xmlDebugDumpNodeXML(xmlNodePtr cur) {
  /* caller frees result, useful for debugging */
  xmlChar *xml;
  xmlBufferPtr buf;

  buf = xmlBufferCreate();
  xmlNodeDumpSecure(buf, cur->doc, cur, 0, 0, 0, 0, 0, 0);
  xml = xmlStrdup(buf->content);
 ...
 */
void
xmlDebugDumpOneNode(FILE * output, xmlNodePtr node, int depth, int include_attributes)
{
    xmlDebugCtxt ctxt;
    ...
    ctxt.output = output;
    ctxt.depth = depth;
    xmlCtxtDumpOneNode(&ctxt, node, include_attributes);
    xmlCtxtDumpCleanCtxt(&ctxt);
}
 ...
 * @include_text: text nodes
 * 
 * @author:  Annesley
 *
 * Dumps 1 level debug information for the nodeset
 */
void xmlDebugDumpNodeSet(FILE *output, xmlNodeSetPtr nodeset, int include_text) {
    xmlDebugCtxt ctxt;
    int i;
    xmlNodePtr cur, prev_ancestor,
               prev_cur = NULL;
    const char *direction_was = " ";
    ...
    for (i = 0; i < nodeset->nodeNr; i++) {
      ...
      if ((include_text == 1) || ((cur->type != XML_TEXT_NODE) && (cur->type != XML_CDATA_SECTION_NODE))) {
        /* work out the move: cur always has a value */
        if (prev_cur != NULL) {
          if      (cur == prev_cur)                   direction_was = "=";
          else if (cur == (xmlNodePtr) prev_cur->properties) direction_was = "@";
          else if (cur == prev_cur->parent)           direction_was = "↥"; /* natural parent */
          else if (cur == prev_cur->next)             direction_was = "↦";
          else if (cur == prev_cur->prev)             direction_was = "↤";
          else if (cur->parent == prev_cur->parent)   direction_was = "⇄"; /* ignored TEXT siblings can cause this */
          else if (prev_cur == cur->parent)           direction_was = "↧";
          else if ((prev_cur->parent != NULL) && (cur->parent == prev_cur->parent->parent)) direction_was = "↱";
          else if (xmlIsParentOf(cur, prev_cur) == 1) direction_was = "⇡"; /* hardlinked parent */
          else {
            direction_was = "?";
            prev_ancestor = prev_cur->parent;
            while ((prev_ancestor != NULL) && (direction_was == "?")) {
              if      (prev_ancestor == cur)          direction_was = "↟"; /* ancestor */
              else if (prev_ancestor == cur->parent)  direction_was = "↬"; /* ancestor -> sibling */
              prev_ancestor = prev_ancestor->parent;
            }
          }
        }
        fprintf(ctxt.output, "%s ", direction_was);
        
        /* dump the node */
        xmlCtxtDumpOneNode(&ctxt, cur, 0);
      
        /* parent_routes */
        /*
        if ((nodeset->parent_routes != NULL) && (nodeset->parent_routes[i]->nodeNr != 0)) {
          nodeset->parent_routes[i]->nodeNr
        }
        */
 ...
 * @depth:  the indentation level.
 *
 * Check the element node, it is recursive
 *
 * Returns the number of errors found
 */
int
xmlDebugCheckNode(FILE * output, xmlNodePtr node, int depth)
{
    xmlDebugCtxt ctxt;
            ...
            xmlNodePtr node2 ATTRIBUTE_UNUSED)
{
    ...
    if ((node->type == XML_DOCUMENT_NODE) ||
        ...
        xmlDebugDumpOneNode(ctxt->output, node, 0, 0);
    }
    return (0);
}
