...
xmlShellReadline(char *prompt) {
    ...
    ret = (char *) malloc(len + 1); /* xmlShellReadline */
    if (ret != NULL) {
...
static void parseAndPrintFile(char *filename, xmlParserCtxtPtr rectxt) {
	...
	if (!debug) {
		...
		if (size >= 0) {
		    UNUSED_RESULT(write(1, result, size)); /* warn_unused_result */
		    xmlFree(result);
		} else {
		...
		if (result == NULL) {
		    ...
		    UNUSED_RESULT(write(1, result, len)); /* warn_unused_result */
		    xmlFree(result);
		}
...
{
    node->_private = malloc(sizeof(long)); /* registerNode */
