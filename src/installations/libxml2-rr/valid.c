...
xmlAddID(xmlValidCtxtPtr ctxt, xmlDocPtr doc, const xmlChar *value,
    ...
    /* Annesley: changing to forcing hard errors here for this situation */
    if (doc == NULL) {
        xmlGenericError(xmlGenericErrorContext, "xmlAddID: doc == NULL\n");
        return(NULL);
    }
    if (value == NULL) {
        xmlGenericError(xmlGenericErrorContext, "xmlAddID: value == NULL\n");
        return(NULL);
    }
    if (attr == NULL) {
        xmlGenericError(xmlGenericErrorContext, "xmlAddID: attr == NULL\n");
        return(NULL);
    }
 ...
 * @user1:  Value supplied by the user
 *
 * Returns 0 to abort the walk or 1 to continue
 */
static void*
xmlWalkRemoveRef(const void *data, const void *user1, ATTRIBUTE_UNUSED const void *user2)
{
    xmlAttrPtr attr0 = ((xmlRefPtr)data)->attr;
    xmlAttrPtr attr1 = ((xmlRemoveMemoPtr)user1)->ap;
    xmlListPtr ref_list = ((xmlRemoveMemoPtr)user1)->l;
...
xmlAddRef(xmlValidCtxtPtr ctxt, xmlDocPtr doc, const xmlChar *value,
    ...
    if (NULL == (ref_list = xmlHashLookup(table, value))) {
        if (NULL == (ref_list = xmlListCreate((xmlListDeallocator) xmlFreeRef, xmlDummyCompare))) {
            xmlErrValid(NULL, XML_ERR_INTERNAL_ERROR,
...
xmlRemoveRef(xmlDocPtr doc, xmlAttrPtr attr) {
    ...
    xmlListWalk(ref_list, xmlWalkRemoveRef, &target, NULL);

    /*If the list is empty then remove the list entry in the hash */
    if (xmlListIsEmpty(ref_list))
                        ...
                        xmlNodePtr elem, xmlAttrPtr attr, const xmlChar *value)
{
    ...
    /* Annesley: check xml:id area policy */
    if (attrDecl->atype == XML_ATTRIBUTE_ID && xmlXMLIDPolicy(attr->parent) == 0) {
...
xmlNodePtr elem, const xmlChar *prefix, xmlNsPtr ns, const xmlChar *value) {
    ...
    /* Validity Constraint: ID uniqueness */
    /* Annesley: check xml:id area policy */
    if (attrDecl->atype == XML_ATTRIBUTE_ID && xmlXMLIDPolicy(elem) == 0) {
 ...
 */
static void*
xmlWalkValidateList(const void *data, const void *user1, ATTRIBUTE_UNUSED const void *user2)
{
        xmlValidateMemoPtr memo = (xmlValidateMemoPtr)user1;
...
xmlValidateCheckRefCallback(xmlListPtr ref_list, xmlValidCtxtPtr ctxt,
    ...
    xmlListWalk(ref_list, xmlWalkValidateList, &memo, NULL);
