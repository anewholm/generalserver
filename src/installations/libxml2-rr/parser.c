                   ...
                   const xmlChar * localname)
{
    ...
    if (prefix == NULL)
        __xmlRaiseError(NULL, NULL, NULL, ctxt, NULL, XML_FROM_PARSER,
                        ...
                        "Attribute %s redefined on %s->%s\n", localname,
                        ctxt->node && ctxt->node->parent ? ctxt->node->parent->name : 0,
                        ctxt->node ? ctxt->node->name : 0);
    else
        __xmlRaiseError(NULL, NULL, NULL, ctxt, NULL, XML_FROM_PARSER,
                        XML_ERR_ATTRIBUTE_REDEFINED, XML_ERR_FATAL, NULL, 0,
                        (const char *) prefix, (const char *) localname,
                        NULL, 0, 0, "Attribute %s:%s redefined on %s\n", prefix,
                        localname, (const char*) (ctxt->node ? ctxt->input : 0));
    if (ctxt != NULL) {
        ctxt->wellFormed = 0;
        if (ctxt->recovery == 0)
            ctxt->disableSAX = 1;
...
#define CMP11( s, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11 ) \
  ( CMP10( s, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10 ) && \
    ((unsigned char *) s)[ 10 ] == c11 )
#define CMP12( s, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12 ) \
  ( CMP11( s, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11 ) && \
    ((unsigned char *) s)[ 11 ] == c12 )
#define CMP13( s, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12, c13 ) \
  ( CMP12( s, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12 ) && \
    ((unsigned char *) s)[ 12 ] == c13 )
...
static int areBlanks(xmlParserCtxtPtr ctxt, const xmlChar *str, int len,
    ...
    lastChild = ctxt->node->last; /* Annesley: was xmlGetLastChild(...) */
    if (lastChild == NULL) {
...
xmlParseNameComplex(xmlParserCtxtPtr ctxt) {
    ...
    if ((*ctxt->input->cur == '\n') && (ctxt->input->cur[-1] == '\r'))
        return(xmlDictLookup(ctxt->dict, ctxt->input->cur - (len + 1), len)); /* xmlParseNameComplex DICT */
...
xmlParseName(xmlParserCtxtPtr ctxt) {
    ...
    if (((*in >= 0x61) && (*in <= 0x7A)) ||
        ...
        if ((*in > 0) && (*in < 0x80)) {
            ...
            ret = xmlDictLookup(ctxt->dict, ctxt->input->cur, count); /* xmlParseName DICT */
            ctxt->input->cur = in;
            ctxt->nbChars += count;
...
xmlParseNCNameComplex(xmlParserCtxtPtr ctxt) {
    ...
    return(xmlDictLookup(ctxt->dict, ctxt->input->cur - len, len)); /* xmlParseNCNameComplex DICT */
}
...
xmlParseNCName(xmlParserCtxtPtr ctxt) {
    ...
    if (((*in >= 0x61) && (*in <= 0x7A)) ||
        ...
        if ((*in > 0) && (*in < 0x80)) {
            count = in - ctxt->input->cur;
            ret = xmlDictLookup(ctxt->dict, ctxt->input->cur, count);/* xmlParseNCName DICT */
            ctxt->input->cur = in;
            ctxt->nbChars += count;
            ctxt->input->col += count;
            if (ret == NULL) {
...
xmlParseQName(xmlParserCtxtPtr ctxt, const xmlChar **prefix) {
    ...
    if (CUR == ':') {
        ...
        if (l == NULL) {
            ...
            p = xmlDictLookup(ctxt->dict, tmp, -1); /* xmlParseQName DICT */
        ...
        if (CUR == ':') {
            ...
            if (tmp != NULL) {
                ...
                l = xmlDictLookup(ctxt->dict, tmp, -1); /* xmlParseQName DICT */
                if (tmp != NULL) xmlFree(tmp);
                *prefix = p;
                return(l);
            }
    ...
    while ((RAW != '>') &&
        ...
        if ((attname != NULL) && (attvalue != NULL)) {
            ...
            if ((attname == ctxt->str_xmlns) && (aprefix == NULL)) {
                const xmlChar *URL = xmlDictLookup(ctxt->dict, attvalue, len); /* xmlParseStartTag2 @xmlns:* URL DICT */
                xmlURIPtr uri;
...
xmlParseBalancedChunkMemoryInternal(xmlParserCtxtPtr oldctxt,
    ...
    if (ctxt->myDoc != NULL) {
        xmlUnlinkNode(ctxt->myDoc->children); /* Annesley: debug locking checks for parent */
...
xmlParseInNodeContext(xmlNodePtr node, const char *data, int datalen,
    ...
    if (node->type == XML_ELEMENT_NODE) {
        ...
        while ((cur != NULL) && (cur->type == XML_ELEMENT_NODE)) {
            ...
            while (ns != NULL) {
                if (ctxt->dict) {
                    iprefix = xmlDictLookup(ctxt->dict, ns->prefix, -1); /* xmlParseInNodeContext namespaces DICT */
                    ihref = xmlDictLookup(ctxt->dict, ns->href, -1);     /* xmlParseInNodeContext namespaces DICT */
                } else {
                    iprefix = ns->prefix;
                    ihref = ns->href;
                }
