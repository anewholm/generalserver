...
{
    return isctype( ch, desc.m_ctype );
}
...
public:
	typedef typename match_char<IterT>::traits_type       traits_type;
	typedef typename match_char<IterT>::char_type         char_type;
...
private:
    ...
    static bool eq( typename match_char<IterT>::char_type left, char_nocase<typename match_char<IterT>::char_type> right )
    {
        return traits_type::eq( left, right.m_chlo ) ||
               traits_type::eq( left, right.m_chhi );
    }
    static void _do_peek_this( peek_param<char_type> & peek, typename match_char<IterT>::char_type ch )
    {
        peek.m_cchars           = 1;
        peek.m_rgchars[0]       = ch;
        peek.m_must_have.m_has  = false;
    }
    static void _do_peek_this( peek_param<char_type> & peek, char_nocase<typename match_char<IterT>::char_type> ch )
    {
        peek.m_cchars           = 2;
...
{
    //C++11 unique_ptr cannot be copy constructed
    //so we changed it to a shared_ptr
    //http://en.cppreference.com/w/cpp/memory/unique_ptr
    typedef std::shared_ptr<custom_charset const> auto_charset;
...
public:
	...
	virtual bool peek_this( peek_param<typename match_group_base<IterT>::char_type> & peek ) const
    {
        if( size_t( -1 ) == this->m_cgroup )
            return false;
...
public:
	...
	virtual bool peek_this( peek_param<typename independent_group<IterT>::char_type> & peek ) const
    {
        return this->next()->peek_this( peek );
    }
};
...
public:
	...
	virtual bool peek_this( peek_param<typename independent_group_base<IterT>::char_type> & peek ) const
    {
        return this->next()->peek_this( peek );
    }
...
struct assertion_condition
{
    ...
    //so we changed it to a shared_ptr
    //http://en.cppreference.com/w/cpp/memory/unique_ptr
    std::shared_ptr<match_group_base<IterT> > m_passert;

#if __cplusplus >= 201103L
    //C++11 introduced deleted and defaulting implicit constructors
    //https://msdn.microsoft.com/en-us/library/dn457344.aspx
    assertion_condition( assertion_condition<IterT> & ac ) 
        : m_passert( ac.m_passert )
    {}
#endif
    ...
    regex_arena & arena )
{
    ...
    return new( arena ) match_conditional<IterT, assertion_condition<IterT> >(cgroup, cond, arena );
}
...
REGEXPR_H_INLINE void basic_rpattern_base<IterT, SyntaxT>::init( string_type const & lpat, REGEX_FLAGS lflags, REGEX_MODE lmode )
{
    basic_rpattern_base<IterT, SyntaxT> temp( lpat, lflags, lmode );
    swap( temp );
}

template< typename IterT, typename SyntaxT >
REGEXPR_H_INLINE void basic_rpattern_base<IterT, SyntaxT>::init( string_type const & lpat, string_type const & lsubst, REGEX_FLAGS lflags, REGEX_MODE lmode )
{
    basic_rpattern_base<IterT, SyntaxT> temp( lpat, lsubst, lflags, lmode );
    swap( temp );
}

template< typename IterT, typename SyntaxT >
REGEXPR_H_INLINE void basic_rpattern_base<IterT, SyntaxT>::_common_init( REGEX_FLAGS lflags )
{
    this->m_cgroups = 0;
    std::vector<detail::match_group_base<IterT>*> rggroups;
    typename string_type::iterator ipat = this->m_pat->begin();
    syntax_type sy( lflags );
	...
	REGEX_ASSERT( 0 == detail::basic_rpattern_base_impl<IterT>::m_pfirst );
	detail::basic_rpattern_base_impl<IterT>::m_pfirst = pgroup;

    // Calculate the width of the pattern and all groups
	this->m_nwidth = pgroup->group_width( rggroups, detail::basic_rpattern_base_impl<IterT>::m_invisible_groups );
    ...
    {
        this->m_flags = ( REGEX_FLAGS ) ( detail::basic_rpattern_base_impl<IterT>::m_flags & ~RIGHTMOST );
        this->m_floop = false;
    }
...
{
    using std::swap;
    std::unique_ptr<string_type> temp_subst( new string_type( subst ) );
    detail::subst_list_type temp_subst_list;
    bool uses_backrefs = false;

    this->_normalize_string( *temp_subst );
...
inline detail::match_group_base<IterT> * basic_rpattern_base<IterT, SyntaxT>::_find_next_group(
    ...
    std::vector<detail::match_group_base<IterT>*> & rggroups )
{
    std::unique_ptr<detail::match_group_base<IterT> > pgroup;
    ...
    if( this->m_pat->end() != ipat && NO_TOKEN != ( tok = sy.ext_token( ipat, this->m_pat->end() ) ) )
    {
        ...
        if( END_GROUP != sy.reg_token( itemp = ipat, this->m_pat->end() ) )
        {
            ...
            case EXT_NOBACKREF:
                ...
                detail::basic_rpattern_base_impl<IterT>::m_invisible_groups.push_back( this->m_cgroups );
            ...
            case EXT_CONDITION:
                ...
                else
                {
                    ...
                    case EXT_NEG_LOOKBEHIND:
                        {
                            std::unique_ptr<detail::match_group_base<IterT> > pgroup_tmp(
    ...
    if( 0 != pgroup.get() )
    {
        ...
        if( detail::is_random_access<IterT>::value && must.m_has && 0 == pgroup->group_number() )
        {
            ...
            detail::basic_rpattern_base_impl<IterT>::m_search = new( this->m_arena ) detail::boyer_moore<iter_type>
    ...
    SyntaxT & sy )
{
    ...
    do
    {
        ...
        if( CHARSET_RANGE == tok && fhave_prev )
        {
            ...
            case CHARSET_RANGE:
            case CHARSET_NEGATE:
                icur = iprev2;  ATTRIBUTE_FALLTHROUGH; // un-get these tokens and fall through
            case NO_TOKEN:
...
inline charset const * get_altern_charset( CharT ch, SyntaxT & sy )
{
    ...
    if( charset_map.end() != iter )
    {
        ...
        if( 0 == pcharset )
        {
            ...
            std::unique_ptr<charset> pnew( new charset );
            std::unique_ptr<charset const> pold( tmp.m_rgcharsets[ !fnocase ] );
...
quantify:   if( ibegin != --ipat )
            ...
            std::unique_ptr<detail::sub_expr<IterT> > pnew( detail::create_char<IterT>( *ipat++, sy.get_flags(), this->m_arena ) );
...
inline bool basic_rpattern_base<IterT, SyntaxT>::_find_next(
    ...
    std::vector<detail::match_group_base<IterT>*> & rggroups )
{
    std::unique_ptr<detail::sub_expr<IterT> > pnew;
    std::unique_ptr<detail::custom_charset> pcs;
    ...
    syntax_type & sy )
{
    if( this->m_pat->end() != ipat && ! pnew->is_assertion() )
    {
        ...
        {
        case ZERO_OR_MORE_MIN:
            fmin = true; ATTRIBUTE_FALLTHROUGH;
        case ZERO_OR_MORE:
            lbound = 0;
            break;
        ...
        if( ( size_t )-1 != lbound )
        {
            ...
            std::unique_ptr<detail::sub_expr<IterT> > pquant( pnew->quantify( lbound, ubound, ! fmin, this->m_arena ) );
            pnew.release();
