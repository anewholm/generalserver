...
template< bool f > struct static_assert_custom;
template<>         struct static_assert_custom<true> { static_assert_custom() {} };
...
// alignof_custom structure
#pragma pack( push )
#pragma pack() // use the default packing
#endif

template< typename T >
class alignof_custom
{
    struct helper
    {
        helper();
...
template< bool > struct destroyer_helper
{
    ...
    {
        //pT, pT->~T(); //left operand has no effect in expression programming
        pT->~T();
    }
};
...
public:
    ...
    ~hetero_stack() // throw()
    {
        ...
        detail::static_assert_custom< detail::is_power_of_two<AlignmentT>::value > const align_test;
    ...
    inline void push( T const & t ) // throw(std::bad_alloc,...)
    {
        ...
        detail::static_assert_custom<( AlignmentT >= detail::alignof_custom<T>::value )> const align_test;
    ...
    {
        detail::static_assert_custom< RuntimeTypeCheckT > const type_check;
