...
//
template< typename T >
inline void swap_auto_ptr( std::unique_ptr<T> & left, std::unique_ptr<T> & right )
{
#if __cplusplus >= 201103L
    left.swap(right);
#else
    std::unique_ptr<T> temp( left );
    left  = right;
    right = temp;
#endif
}
...
template< typename T >
inline void reset_auto_ptr( std::unique_ptr<T> & left )
{
#if __cplusplus >= 201103L
    left.release();
#else
    std::unique_ptr<T> temp( (T*) 0 );
    left = temp;
#endif
}

template< typename T, typename U >
inline void reset_auto_ptr( std::unique_ptr<T> & left, U * right )
{
#if __cplusplus >= 201103L
    left.reset(right);
#else
    std::unique_ptr<T> temp( right );
    left = temp;
#endif
}
...
inline void * pool_impl<AllocT>::allocate( size_t size )
{
    ...
    m_data.m_pfirst->m_offset += ( ( size + alignof_custom<not_pod>::value - 1 )
                                 & ~( alignof_custom<not_pod>::value - 1 ) );
...
#if __cplusplus >= 201103L
    static void * operator new( size_t size ); // throw( std::bad_alloc ); // depreceated in C++11
    // disable all the vector new's and delete's.
    static void * operator new[]( size_t size, regex_arena & arena ); // throw( std::bad_alloc ); // depreceated in C++11
    static void operator delete[]( void *, regex_arena & );
    static void * operator new[]( size_t size ); // throw( std::bad_alloc ); // depreceated in C++11
    static void operator delete[]( void * );
#else
...
protected:
    ...
    std::unique_ptr<string_type>   m_pat;   // contains the unparsed pattern
    std::unique_ptr<string_type>   m_subst; // contains the unparsed substitution
...
struct regex_access
{
    ...
    )
    {
        ...
        while( 1 != limit && _do_try_match( pat, param, rgbackrefs, use_null ) )
        {
            ...
            {
                backref_type const & brG = rgbackrefs[i];
                string_type tmpG( brG.first, brG.second, char_allocator );
                results.strings().push_back( tmpG );
            }
    ...
    {
        /* typedef std::basic_string<CharT, TraitsT, AllocT> string_type; Annesley; UNUSED */ 
