...
//{
    ...
    //static static_assert_custom<32==sizeof(backref_tag<char*>)> const check_backref_size;
//}
...
protected:
    ...
    (
        string_type const & lpat,
        REGEX_FLAGS lflags = NOFLAGS,
        REGEX_MODE lmode = MODE_DEFAULT
    )                                                   //throw( bad_regexpr, std::bad_alloc )
        : detail::basic_rpattern_base_impl<IterT>( lflags, lmode, lpat )
    {
        _common_init( this->m_flags );
    }
    ...
    (
        string_type const & lpat,
        string_type const & lsubst,
        REGEX_FLAGS lflags = NOFLAGS,
        REGEX_MODE lmode = MODE_DEFAULT
    )                                                   //throw( bad_regexpr, std::bad_alloc )
        : detail::basic_rpattern_base_impl<IterT>( lflags, lmode, lpat, lsubst )
    {
        _common_init( this->m_flags );
        this->_normalize_string( *this->m_subst );
        _parse_subst( *this->m_subst, this->m_fuses_backrefs, this->m_subst_list ); // must come after _common_init
    }
	...
	// Annesley: required to avoid non-tempate parameter function warnings
    size_t _get_next_group_nbr()
    {
		return detail::basic_rpattern_base_impl<IterT>::m_cgroups++;
    }
...
public:
    ...
    ) const
    {
        ...
        detail::static_assert_custom< detail::is_convertible<OtherT,IterT>::value > const iterator_types_are_not_convertible;
    ...
    ) const
    {
        ...
        // convertible to type IterT. Check the declaration of your rpattern object.
        detail::static_assert_custom< detail::is_convertible<CharT*,IterT>::value > const iterator_types_are_not_convertible;
        ( void ) iterator_types_are_not_convertible;
    ...
    ) const
    {
        ...
        detail::static_assert_custom< detail::is_convertible<iter_type,IterT>::value > const iterator_types_are_not_convertible;
        ( void ) iterator_types_are_not_convertible;

        IterT ibegin = str.begin(), iend = str.begin();

		if( len == basic_rpattern_base<IterT, SyntaxT>::npos || pos + len >= str.size() )
            iend = IterT(str.end());
        else
    ...
    ) const
    {
        ...
        // convertible to type IterT. Check the declaration of your rpattern object.
        detail::static_assert_custom< detail::is_convertible<Char1T*,IterT>::value > const iterator_types_are_not_convertible;
        ( void ) iterator_types_are_not_convertible;
